...
def _ensure_accounts_tab():
    ...
    Cols A–O per plan §2.1. Bank/debit/cash use F+G (starting + running),
    credit uses H+I (limit + outstanding); J+K (statement/due day) credit-only.
    """
    ss = _get_spreadsheet()
    try:
        ...
        ws.update("A1:O1", [ACCOUNTS_HEADER])
...
def _is_pending_expired(created_at_iso: str) -> bool:
    ...
    try:
        ...
        # Compare as naive UTC (created_at is utcnow().isoformat())
        if created.tzinfo is not None:
            created = created.astimezone(tz=None).replace(tzinfo=None)
        delta = (_dt.utcnow() - created).total_seconds()
...
def _set_pending_status(setup_key: str, status: str) -> bool:
    """Internal: update status + completed_at for the row matching setup_key."""
    if not setup_key:
        return False
    ws = _ensure_pending_accounts_tab()
    rows = ws.get_all_values()[1:]
