...
> **Trạng thái:** Draft
> **Tham chiếu:** [brd.md](file:///Users/maingocanh/Projects/My Money Went/docs/brd-vi.md) v2.8.0
...
### 1.1. Mô tả
My Money Went là Telegram bot SaaS tự động theo dõi tài chính cá nhân và shop nhỏ. Bot kết nối ngân hàng qua **3 entry path** (SePay quick connect, SePay wizard, Email forwarding), nhận giao dịch real-time, hỏi user phân loại qua inline buttons, và tổng hợp báo cáo tự động. Cấu trúc 3-tier pricing (Free / Pro $4 / Business $9).
...
| Payment | **Bank transfer + auto-detect** qua SePay primary + Email backup (xem [feature_payment.md](file:///Users/maingocanh/Projects/MyMoneyWent/docs/features/feature_payment.md)) · PayPal/USDT defer Phase 2 |
| Backup | Backblaze B2 daily với SSE-B2 server-side encryption + `pg_dumpall --globals-only` cho roles |
| Admin tools | Telegram commands restricted bằng `ADMIN_TELEGRAM_IDS` (comma-separated, multi-admin), rate limit `ADMIN_RATE_LIMIT_PER_MIN=30` default. `/admin_help` hybrid auto-generated từ `@admin_only` registry. Detail: [feature_admin_tools.md](file:///Users/maingocanh/Projects/MyMoneyWent/docs/features/feature_admin_tools.md) |
| Observability | Sentry + Railway metrics + UptimeRobot. Founder daily dashboard `/admin_stats`, cost dashboard `/admin_cost`, per-user troubleshooter `/admin_user`. Error budget 0.1% policy. Detail: [observability-plan v1.1.0](file:///Users/maingocanh/Projects/My Money Went/docs/observability-plan.md) |
| Disaster recovery | 8 scenarios documented + quarterly drill. RTO 2-4h, RPO 24h. `BOT_TOKEN_BACKUP` ready, `@MyMoneyWentUpdates` channel for out-of-band notification. Detail: [DR runbook v1.1.0](file:///Users/maingocanh/Projects/My Money Went/docs/runbooks/disaster-recovery.md) |

### 1.4. Bot ownership model

**1 shared bot duy nhất do platform sở hữu** (vd `@MyMoneyWentBot`). User KHÔNG cần tạo bot riêng, không cần biết `BOT_TOKEN`, không cần lookup `chat_id` của mình.

**Flow đăng ký:**

1. User search `@MyMoneyWentBot` trên Telegram → bấm "Start"
2. Bot nhận `update.message.from.id` = user's `telegram_id`
3. Backend: `INSERT INTO users (telegram_id, ...) ON CONFLICT DO NOTHING`
4. Bot reply về đúng `chat_id` của user đó (lấy từ `update.message.chat.id`, lưu trong `users.chat_id`)

**Rationale:** Multi-tenant SaaS UX phải chuẩn "thêm bạn → dùng ngay". Bắt user tự tạo bot qua @BotFather = mất 30-60 phút setup, defeat 2-15 phút onboarding promise của 3-path flow.

**Operational implications:**
- `BOT_TOKEN` là **1 ENV var duy nhất ở Railway**, do platform owner quản lý + rotate khi cần.
- `chat_id` luôn lookup từ DB qua `users.telegram_id`, **KHÔNG hardcode** trong code/env.
- Mọi `send_text(text, chat_id=...)` phải truyền `chat_id` explicit từ context — không có default global.
- Single point of failure: nếu Telegram suspend bot (rare nhưng có thể vì abuse): toàn user offline. Mitigation: maintain 1 backup bot token sẵn (`BOT_TOKEN_BACKUP` env var) cho emergency switchover < 5 phút.

**Acceptance Criteria liên quan (cross-ref F01, F08):**
- [ ] `BOT_TOKEN` chỉ tồn tại trong Railway env vars, không commit code
- [ ] Mọi outbound Telegram call phải resolve `chat_id` qua `users` table, không có hardcoded fallback
- [ ] `users.telegram_id` UNIQUE constraint — 1 telegram account = 1 user row
- [ ] `users.chat_id` cập nhật từ mỗi inbound update (user có thể block + unblock bot, chat_id không đổi nhưng cần verify still valid)
- [ ] Bot suspended scenario có runbook (manual: rotate `BOT_TOKEN` env var ở Railway, restart)
...
```
User tìm @MyMoneyWentBot trên Telegram
    │
    ▼
User gửi /start
...
Bot gửi welcome + chọn path:
    ...
    │   ├── Bot hiện webhook URL: https://api.mymoneywent.com/hook/{token}
...
Bot gửi step-by-step wizard:
    ...
    │
    Step 3: "Vào phần Webhook → dán URL này: https://api.mymoneywent.com/hook/{token}"
            [✅ Đã dán] [❓ Cần hỗ trợ]
    │
    ▼
...
Bot gửi:
    ...
    │
    ▼
Bot gửi: "✅ Đã tạo email u{id}@in.mymoneywent.com
...
**Payment flow:** Detail spec ở [feature_payment.md](file:///Users/maingocanh/Projects/MyMoneyWent/docs/features/feature_payment.md). Tóm tắt:
- User `/upgrade` → bot tạo `pending_payment` với ref `PAY-{user_id}-{plan}-{period}-{nonce4}`
- User chuyển khoản tới 1 trong 2 platform bank account (VCB primary linked SePay + TCB secondary email-only)
- Auto-detect: SePay webhook ≤ 60s p95, Email backup ≤ 5min p95
...
| Inbound Email | Hiển thị `u{id}@in.mymoneywent.com` | Auto-generated |
...
| **Error budget** | ≤0.1% error rate rolling 30-day window. Policy 3-tier (>50% remain → ship features / 0-50% → slow down / <0% → freeze). Detail: [observability §4b](file:///Users/maingocanh/Projects/My Money Went/docs/observability-plan.md) |
| **Disaster recovery** | RTO ≤2h (DB corruption), ≤4h (Railway outage), ≤5min (BOT_TOKEN compromise). 8 scenarios documented: [DR runbook](file:///Users/maingocanh/Projects/My Money Went/docs/runbooks/disaster-recovery.md) |
| **Out-of-band notification** | `@MyMoneyWentUpdates` Telegram channel (created pre-launch) for bot suspension scenarios |
...
| **inbound_email** | Email address dạng u{id}@in.mymoneywent.com cho email forwarding |
...
### 7.2. References
- [BRD v2.8.0](file:///Users/maingocanh/Projects/My Money Went/docs/brd-vi.md)
- [TDD](file:///Users/maingocanh/Projects/My Money Went/docs/tdd.md)
- [Feature: Personal vs Business toggle](file:///Users/maingocanh/Projects/MyMoneyWent/docs/features/feature_personal_business_toggle.md)
- [Feature: Refactor personal → SaaS multi-tenant](file:///Users/maingocanh/Projects/MyMoneyWent/docs/features/feature_saas_refactor.md)
- [Feature: Payment via bank transfer](file:///Users/maingocanh/Projects/MyMoneyWent/docs/features/feature_payment.md)
- [Feature: Admin tools & audit](file:///Users/maingocanh/Projects/MyMoneyWent/docs/features/feature_admin_tools.md)
- [Runbook: Disaster recovery](file:///Users/maingocanh/Projects/My Money Went/docs/runbooks/disaster-recovery.md)
- [Observability plan](file:///Users/maingocanh/Projects/My Money Went/docs/observability-plan.md)
- [Implementation plan: scale to 500 users and more](file:///Users/maingocanh/Projects/My Money Went/docs/implementation-plan-500-users-and-more.md)
...
| v1.5.0 | 2026-05-06 | **Sync với 3 spec mới + BRD v2.8.0:** (1) §1.3 tech stack thêm 3 row: **Admin tools** (env `ADMIN_TELEGRAM_IDS`, `ADMIN_RATE_LIMIT_PER_MIN=30`, `/admin_help` registry hybrid), **Observability** (Sentry + Railway, error budget 0.1%, dashboards `/admin_stats` `/admin_cost` `/admin_user`), **Disaster recovery** (8 scenarios, RTO 2-4h, BOT_TOKEN_BACKUP, `@MyMoneyWentUpdates` channel). Backup row clarify SSE-B2 + `pg_dumpall --globals-only`. (2) §4.2 Key Tables thêm 5 row: `pending_payments`, `payment_matches`, `unmatched_payments`, `admin_audit_log`, `analytics_events`. (3) §5.3 Reliability NFR thêm 3 metric: error budget, disaster recovery RTO, out-of-band notification channel. Backup row clarify globals + SSE-B2. Webhook retry clarify cross-source dedup state machine. (4) §6 Analytics thêm 3 admin events: `admin_command_executed`, `admin_command_denied`, `admin_manual_payment_resolved`. (5) §7.2 References thêm 4 cross-doc link (admin tools, DR runbook, observability, implementation plan). |
