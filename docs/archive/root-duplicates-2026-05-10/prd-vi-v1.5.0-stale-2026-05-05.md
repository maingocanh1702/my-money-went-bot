...
> **Trạng thái:** Draft
> **Tham chiếu:** [brd.md](file:///Users/maingocanh/Projects/Tiền Về Nơi Đâu/docs/brd-vi.md) v2.8.0
...
### 1.1. Mô tả
Tiền Về Nơi Đâu là Telegram bot SaaS tự động theo dõi tài chính cá nhân và shop nhỏ. Bot kết nối ngân hàng qua **3 entry path** (SePay quick connect, SePay wizard, Email forwarding), nhận giao dịch real-time, hỏi user phân loại qua inline buttons, và tổng hợp báo cáo tự động. Cấu trúc 3-tier pricing (Free / Pro $4 / Business $9).
...
| Observability | Sentry + Railway metrics + UptimeRobot. Founder daily dashboard `/admin_stats`, cost dashboard `/admin_cost`, per-user troubleshooter `/admin_user`. Error budget 0.1% policy. Detail: [observability-plan v1.1.0](file:///Users/maingocanh/Projects/Tiền Về Nơi Đâu/docs/observability-plan.md) |
| Disaster recovery | 8 scenarios documented + quarterly drill. RTO 2-4h, RPO 24h. `BOT_TOKEN_BACKUP` ready, `@TienVeNoiDauUpdates` channel for out-of-band notification. Detail: [DR runbook v1.1.0](file:///Users/maingocanh/Projects/Tiền Về Nơi Đâu/docs/runbooks/disaster-recovery.md) |

### 1.4. Bot ownership model

**1 shared bot duy nhất do platform sở hữu** (vd `@TienVeNoiDauBot`). User KHÔNG cần tạo bot riêng, không cần biết `BOT_TOKEN`, không cần lookup `chat_id` của mình.

**Flow đăng ký:**

1. User search `@TienVeNoiDauBot` trên Telegram → bấm "Start"
...
```
User tìm @TienVeNoiDauBot trên Telegram
    │
    ▼
User gửi /start
...
| **Error budget** | ≤0.1% error rate rolling 30-day window. Policy 3-tier (>50% remain → ship features / 0-50% → slow down / <0% → freeze). Detail: [observability §4b](file:///Users/maingocanh/Projects/Tiền Về Nơi Đâu/docs/observability-plan.md) |
| **Disaster recovery** | RTO ≤2h (DB corruption), ≤4h (Railway outage), ≤5min (BOT_TOKEN compromise). 8 scenarios documented: [DR runbook](file:///Users/maingocanh/Projects/Tiền Về Nơi Đâu/docs/runbooks/disaster-recovery.md) |
| **Out-of-band notification** | `@TienVeNoiDauUpdates` Telegram channel (created pre-launch) for bot suspension scenarios |
...
### 7.2. References
- [BRD v2.8.0](file:///Users/maingocanh/Projects/Tiền Về Nơi Đâu/docs/brd-vi.md)
- [TDD](file:///Users/maingocanh/Projects/Tiền Về Nơi Đâu/docs/tdd.md)
...
- [Runbook: Disaster recovery](file:///Users/maingocanh/Projects/Tiền Về Nơi Đâu/docs/runbooks/disaster-recovery.md)
- [Observability plan](file:///Users/maingocanh/Projects/Tiền Về Nơi Đâu/docs/observability-plan.md)
- [Implementation plan: scale to 500 users and more](file:///Users/maingocanh/Projects/Tiền Về Nơi Đâu/docs/implementation-plan-500-users-and-more.md)
...
| v1.5.0 | 2026-05-06 | **Sync với 3 spec mới + BRD v2.8.0:** (1) §1.3 tech stack thêm 3 row: **Admin tools** (env `ADMIN_TELEGRAM_IDS`, `ADMIN_RATE_LIMIT_PER_MIN=30`, `/admin_help` registry hybrid), **Observability** (Sentry + Railway, error budget 0.1%, dashboards `/admin_stats` `/admin_cost` `/admin_user`), **Disaster recovery** (8 scenarios, RTO 2-4h, BOT_TOKEN_BACKUP, `@TienVeNoiDauUpdates` channel). Backup row clarify SSE-B2 + `pg_dumpall --globals-only`. (2) §4.2 Key Tables thêm 5 row: `pending_payments`, `payment_matches`, `unmatched_payments`, `admin_audit_log`, `analytics_events`. (3) §5.3 Reliability NFR thêm 3 metric: error budget, disaster recovery RTO, out-of-band notification channel. Backup row clarify globals + SSE-B2. Webhook retry clarify cross-source dedup state machine. (4) §6 Analytics thêm 3 admin events: `admin_command_executed`, `admin_command_denied`, `admin_manual_payment_resolved`. (5) §7.2 References thêm 4 cross-doc link (admin tools, DR runbook, observability, implementation plan). |
