...
> **Trạng thái:** Draft
> **Owner:** Founder (dev)
> **Mục đích:** Day-by-day actionable plan cho 10-day Messenger channel build trong Phase 6 Tuần 10-11. Bridge gap giữa feature-spec-messenger-channel v1.1.1 (what + why) và actual sprint execution (when + how). Equivalent role với implementation-plan-payment-vietqr-email cho VietQR work.
> **Phase liên quan:** MVP Phase 6 Tuần 10-11 (Messenger code build), parallel với Phase 5 cuối (Email parser TCB/Cake) cho prereq overlap. App Review chạy parallel review window 3-14 ngày.
> **Tham chiếu:** [Feature Spec Messenger v1.1.1](file:///Users/YourName/Projects/MyMoneyWent/docs/features/feature_messenger_channel.md) (canonical spec — what to build) · [BRD v2.9.0 §2.2 mục tiêu 4 + §8 Phase 6](file:///Users/YourName/Projects/MyMoneyWent/docs/brd-vi.md) · [PRD v1.6.0 §1.4](file:///Users/YourName/Projects/MyMoneyWent/docs/prd.md) · [TDD v1.6.0 §2.1 schema + §3.1 endpoints + §5.2 env vars](file:///Users/YourName/Projects/MyMoneyWent/docs/tdd.md) · [Impl Plan VietQR+Email v1.0.0](file:///Users/YourName/Projects/MyMoneyWent/docs/implementation-plan-payment-vietqr-email.md) (couples ở `send_image()`) · [Decision Onboarding UI v1.0.1](file:///Users/YourName/Projects/MyMoneyWent/docs/decision-onboarding-ui-strategy.md) (chat-only, no web form)
...
10 ngày dev (Tuần 10-11 Phase 6) build Messenger channel từ scratch:
...
|---|---|---|
| Public launch timing | Code ship MVP, public access feature-flagged sau Meta App Review approve | [BRD v2.9.0 §2.2 mục tiêu 4](file:///Users/YourName/Projects/MyMoneyWent/docs/brd-vi.md) |
| Channel mode | Single-channel per user (chọn 1 lúc onboarding) | [Messenger spec §1.1](file:///Users/YourName/Projects/MyMoneyWent/docs/features/feature_messenger_channel.md) |
| Onboarding UI | Chat-only (KHÔNG có web form) | [Decision Onboarding UI v1.0.1](file:///Users/YourName/Projects/MyMoneyWent/docs/decision-onboarding-ui-strategy.md) |
| FB Business setup | Founder đã có Page + Business Manager → Day 0 chỉ ~2-4h cho App config | User confirmed |
| Page name | Defer Day 0 Tuần 10 — founder decide trước khi tạo App webhook | User confirmed |
...
| Adapter pattern | `services/channels/{base,telegram,messenger}.py` — base ABC + channel-specific impl. Foundation từ Phase 1-2 refactor | [Refactor spec v1.3.0 §2.2](file:///Users/YourName/Projects/MyMoneyWent/docs/features/feature_saas_refactor.md) |
...
| Web dashboard for Messenger users | Phase 9+ trigger-based | [Decision Onboarding UI §6](file:///Users/YourName/Projects/MyMoneyWent/docs/decision-onboarding-ui-strategy.md) |
...
**10 ngày dev** (5 ngày Tuần 10 + 5 ngày Tuần 11), assumes:

- Founder solo, ~6 productive hours/day
- FB Business Manager + Page existing (Day 0 ~2-4h, không phải full day)
- Privacy policy URL ready trước Day 3 (App Review submit) — **dependency: ship landing/privacy/terms TRƯỚC Tuần 10**, theo [decision-onboarding-ui-strategy §5.2](file:///Users/YourName/Projects/MyMoneyWent/docs/decision-onboarding-ui-strategy.md)
- Phase 1-5 đã ship: schema multi-channel ready, `services/channels/base.py` + `TelegramSender` migrated, email parser TCB/Cake done
- vietqr.io public API responsive (couples Day 6 send_image work)
- Meta App Review chạy parallel — KHÔNG block Tuần 10-11 dev (review approve có thể Phase 7-8 hoặc post-launch, channel feature-flagged)
...
Pointer to canonical [feature-spec-messenger-channel §2-§5](file:///Users/YourName/Projects/MyMoneyWent/docs/features/feature_messenger_channel.md). High-level:
...
**No DDL changes** trong scope plan này. Schema `users.channel_type`, `channel_user_id`, `last_user_message_at`, `invalid_channel`, UNIQUE pair constraint đều **đã ship Phase 1** ([TDD v1.6.0 §2.1](file:///Users/YourName/Projects/MyMoneyWent/docs/tdd.md)).
...
- [ ] **Decide Page name** (15-30 phút) — Q1 trong Messenger spec §14. Suggested: `FinTrack` (single brand với Telegram), alternatives `MyMoneyWent` hoặc Vietnamese name. Verify URL `m.me/{PageName}` available.
- [ ] **Verify privacy policy URL accessible** — `https://fintrack.app/privacy` phải ship trước Day 3. Block dependency on landing/privacy/terms plan ([decision-onboarding-ui-strategy §5.2 + §8](file:///Users/YourName/Projects/MyMoneyWent/docs/decision-onboarding-ui-strategy.md)). Nếu chưa ship: block Day 3 App Review submit.
...
- [ ] (15m) Smoke test với curl: `curl "https://api.fintrack.app/webhook/messenger?hub.mode=subscribe&hub.verify_token=$TOKEN&hub.challenge=test"` → 200 returns "test"
...
- [ ] (1.5h) **Submit Meta App Review** — use case writeup (template trong §8 plan này):
    ...
    - Provide privacy policy URL `https://fintrack.app/privacy`
    - Provide terms URL `https://fintrack.app/terms`
    - Upload app icon
...
- [ ] (45m) Path A — SePay quick connect: bot reply 2 message:
    ...
    - Message 2 (standalone): `https://api.fintrack.app/hook/{user_token}` — long-press để copy
...
- [ ] (1h) Path C — Email forwarding: bot reply 2 message:
    ...
    - Message 2 (standalone): `u{user_id}@in.fintrack.app`
    - Quick replies: [📱 Gmail] [💻 Outlook] [❓ Khác]
    - Each → reply guide instructions

**Afternoon (3h):**

- [ ] (1h) Channel-specific copy templates `copy/onboarding.py` initial — refactor inline strings thành `COPY[key][channel_type]` pattern theo [Messenger spec §7.5](file:///Users/YourName/Projects/MyMoneyWent/docs/features/feature_messenger_channel.md)
...
**Output Day 5 / End Tuần 10:** Core Messenger flow working — onboard + transaction categorize + status. App Review review window open. Outstanding: send_image, MESSAGE_TAG, payment, copy module full, screencast.
...
> **Couples với** [implementation-plan-payment-vietqr-email Day 1-2](file:///Users/YourName/Projects/MyMoneyWent/docs/implementation-plan-payment-vietqr-email.md). Code overlap, not duplicate work.
...
- [ ] (1.5h) Subscription payment Messenger adapter (couples [VietQR plan §6.6](file:///Users/YourName/Projects/MyMoneyWent/docs/implementation-plan-payment-vietqr-email.md)):
...
- [ ] (1h) Verify divergent copy theo [Messenger spec §7.5 UX parity matrix](file:///Users/YourName/Projects/MyMoneyWent/docs/features/feature_messenger_channel.md):
...
- [ ] (1.5h) Privacy policy update Messenger section (theo [Messenger spec §6.7](file:///Users/YourName/Projects/MyMoneyWent/docs/features/feature_messenger_channel.md)):
    ...
    - Deploy update tới `https://fintrack.app/privacy`
...
- [ ] (1h) **Record App Review screencast** (English voiceover):
    - Tool: Loom hoặc OBS
    - Script (~2-3 phút):
        1. "Hi, this is FinTrack — a financial tracking bot for Vietnamese users on Messenger"
...
- [ ] (1h) Performance: bot reply latency p95 < 2s (per [Messenger spec §9.5 AC](file:///Users/YourName/Projects/MyMoneyWent/docs/features/feature_messenger_channel.md))
...
- [ ] (1h) Buffer cho App Review feedback iteration (Meta thường response 3-7 days, có thể fall trong Tuần 11)

**Output Day 10 / End Tuần 11:** Code complete, all AC except App Review pending Meta response. `ENABLE_MESSENGER_CHANNEL=false` mặc định, ready flip ON khi App Review approve.
...
> **Critical:** Day 0 work parallel với Phase 5 cuối (~Tuần 9). Failure to complete blocks Tuần 10 Day 1.

- [ ] **Page name decided** (Q1 deferred trong spec §14)
- [ ] **Landing page + Privacy policy + Terms ship** ở `https://fintrack.app/{,/privacy,/terms}` — depend on [decision-onboarding-ui-strategy §5.2 implementation](file:///Users/YourName/Projects/MyMoneyWent/docs/decision-onboarding-ui-strategy.md), **3 ngày dev separate task** trước Tuần 10
...
```
[00:00-00:15] Intro
"Hi, this is FinTrack — a financial tracking bot for Vietnamese users on Messenger.
...
- Voice over: "All notifications are scoped to user's own data — no cross-user leakage.
  Privacy policy at fintrack.app/privacy details our data handling practices."
...
- [ ] Privacy policy URL: `https://fintrack.app/privacy`
- [ ] Terms URL: `https://fintrack.app/terms`
...
|---|---|---|---|---|
| 1 | App Review reject | Trung bình (50% common, 30% expected for tagged use cases first submit) | KHÔNG block MVP launch (decoupled qua flag). Block public Messenger access | Day 11+ buffer cho iteration. Resubmit 3-7 day cycle. Worst case: Phase 7-8 vẫn approve. Plan B: Telegram-only launch, Messenger flip ON post-launch. |
...
| 3 | Privacy policy URL chưa ship trước Day 3 | Trung bình | Block App Review submit | Founder must ship landing/privacy/terms TRƯỚC Tuần 10 (3 ngày dev separate). Track trong impl plan landing/privacy/terms. |
...
| 5 | Founder solo timeline pressure (10 ngày Tuần 10-11 + parallel VietQR + admin tools) | Cao | Phase 6 slip 1-2 weeks | Sequencing trong §11. VietQR plan absorb couples ở Day 6. Admin tools defer Tuần 12 hoặc post-launch nếu cần. |
...
Phase 6 Tuần 10 — THIS PLAN:
  ...
  ├─ Day 4-5 onboarding 3-path + transaction picker              │
                                                                  ▼
Phase 6 Tuần 11 — THIS PLAN + VietQR plan COUPLED:
  ...
  └─ Day 10 E2E + buffer                                          │
                                                                  ▼
Phase 6 Tuần 12 (NOT this plan):
  ├─ Admin tools commands                                         │
  ├─ Observability dashboard                                      │
  ├─ Production deploy                                             │
  └─ Buffer for App Review reject iteration                       │
                                                                  ▼
Phase 7-8: Beta + soft launch (Telegram primary, Messenger flip if approved)
```

### 11.2. Conflict với VietQR plan

[Implementation Plan VietQR+Email v1.0.0 §5](file:///Users/YourName/Projects/MyMoneyWent/docs/implementation-plan-payment-vietqr-email.md) đặt VietQR work ở Tuần 11 với Day 1-4 breakdown. THIS plan đặt Messenger Day 6-7 cũng ở Tuần 11. **Họ couple, không double-count:**
...
| Day 3 — Email parallel parser + handler | Tuần 12 hoặc parallel với Tuần 11 nếu founder đa task | Separate, no Messenger touch. Defer if founder tập trung Messenger Tuần 11. |
...
VietQR plan Day 3 (email parallel parser/handler) → can ship Tuần 12 hoặc compress vào Tuần 11 nếu founder có capacity. Recommend Tuần 12.

### 11.3. What Tuần 12 looks like (not this plan, but called out)

- Email parallel handler (VietQR plan Day 3 deferred): 1 day
- Admin tools commands (BRD §8 Phase 6.d): 3-5 days
- Observability dashboard (BRD §8 Phase 6.e): 3-5 days
- Production deploy + DNS: 1-2 days
- App Review iteration if Meta rejected during Tuần 10-11: 1-3 days
- **Total: 9-16 days work for 5 days available**

This is over budget. Realistic options:
- Push timeline to 17 weeks (extend Phase 6 to Tuần 13)
...
|---|---|---|---|---|
| 1 | Page name | ⏸️ Defer Day 0 Tuần 10 | Day 0 task — must decide before App config | Day 0 morning |
| 2 | Landing/privacy/terms ship status | ⚠️ Depends separate plan | Day 3 App Review submit | Trước Tuần 10 |
| 3 | App icon source | ⏸️ Defer Day 0 (Fiverr lead time 1-2 days) | Day 3 App Review submit | Day 0 (start Fiverr order) |
| 4 | Tuần 12 timeline conflict resolution | ⚠️ Acknowledged §11.3, founder decision | Phase 6 commitment | Tuần 11 cuối |
...
## 13. Definition of Done (Tuần 11 cuối)
...
- [ ] Privacy policy update deployed `https://fintrack.app/privacy`
...
- [Feature Spec Messenger v1.1.1](file:///Users/YourName/Projects/MyMoneyWent/docs/features/feature_messenger_channel.md) — canonical what to build
- [BRD v2.9.0 §2.2 mục tiêu 4 + §8 Phase 6](file:///Users/YourName/Projects/MyMoneyWent/docs/brd-vi.md) — strategic decision
- [PRD v1.6.0 §1.4 dual channel](file:///Users/YourName/Projects/MyMoneyWent/docs/prd.md) — product flow
- [TDD v1.6.0 §2.1 schema + §3.1 endpoints + §5.2 env vars](file:///Users/YourName/Projects/MyMoneyWent/docs/tdd.md) — technical foundation
- [Implementation Plan VietQR+Email v1.0.0](file:///Users/YourName/Projects/MyMoneyWent/docs/implementation-plan-payment-vietqr-email.md) — couples Day 6-7
- [Decision Onboarding UI v1.0.1](file:///Users/YourName/Projects/MyMoneyWent/docs/decision-onboarding-ui-strategy.md) — chat-only, no web form
- [Disaster Recovery Runbook v1.2.0 §8e Scenario J](file:///Users/YourName/Projects/MyMoneyWent/docs/runbooks/disaster-recovery.md) — Page suspended scenario
- [Implementation Plan 500 users v1.4.0 §C10](file:///Users/YourName/Projects/MyMoneyWent/docs/implementation-plan-500-users-and-more.md) — channel adapter foundation
