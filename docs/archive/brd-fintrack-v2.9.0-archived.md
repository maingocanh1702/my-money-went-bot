...
> **Trạng thái:** Draft (consolidated + 4 round revisions)
>
> **🌐 SCOPE NOTE (added 2026-05-10):** This BRD specifies the **🇻🇳 Vietnam market track only**. The transaction capture stack here (SePay webhook + Vietnamese bank email parsing for TCB/Cake/ACB/STB/BIDV/MB), the 3 personas (Minh/Linh/Hùng+), and the pricing tiers ($4 Pro / $9 Business) are **VN-specific** and remain committed for VN launch. The Global market is a **parallel track** with its own capture stack (Plaid/TrueLayer + Stripe/Shopify/Etsy APIs) — see [strategic-pivot-global.md](../strategic-pivot-global.md). The two tracks share Phase 1-2 multi-tenant foundation; they diverge from Phase 3+. For the side-by-side comparison and how the tracks coexist, read [docs/market-strategy-overview.md](./market-strategy-overview.md) first.
>
> **Thay đổi v2.9 vs v2.8 — MULTI-CHANNEL FOUNDATION MVP:** Messenger từ "Phase 2 validate" → **foundation + code ship MVP Phase 1–6, public launch feature-flagged sau Meta App Review approve**. Telegram public launch theo timeline 16 tuần — không phụ thuộc Meta. Single-channel per user (khi cả 2 channel live, chọn 1 lúc onboarding). Schema multi-channel ship Phase 1, integration code Tuần 10–11. Thêm **3-5 ngày dev** cho VietQR display via vietqr.io public image URL + email-based parallel payment path qua TCB/MB secondary bank (integration/test surface lớn — không phải 2 ngày). Timeline 16 tuần MVP giữ nguyên, cost monthly $0 net.
...
### 1.1. Tên sản phẩm
**FinTrack** — Telegram Bot tự động theo dõi tài chính cá nhân và shop nhỏ qua SePay.

### 1.2. Tầm nhìn (Vision)
Biến việc quản lý tài chính từ "mở app ngân hàng → ghi chép thủ công → quên sau 1 tuần" thành **low-effort tracking trong messaging app**: giao dịch xảy ra → bot hỏi phân loại → bấm 1 nút → P&L cập nhật real-time.
...
Bot Telegram kết nối SePay → **tự động nhận giao dịch ngân hàng** → user phân loại qua nút bấm → tổng hợp báo cáo tự động. Cấu trúc 3 tier (Free / Pro / Business) phục vụ từ user cá nhân đến chủ shop nhỏ.
...
**1 shared bot do platform sở hữu** — KHÔNG bắt user tự tạo bot Telegram. Đây là quyết định kiến trúc cốt lõi của SaaS:

| Aspect | Self-hosted (current) | SaaS shared bot (target) |
|---|---|---|
| Bot creation | User tự tạo qua @BotFather | Platform tạo 1 lần, mọi user dùng chung |
| BOT_TOKEN | User tự manage | Platform owner manage trong Railway env |
| Deployment | Mỗi user 1 instance | 1 instance, multi-tenant DB |
| Setup time | 30-60 phút (8 bước) | 2-15 phút (1 trong 3 path) |
| User identification | Hardcode `CHAT_ID` env | Lookup `users.telegram_id` từ DB |
| TAM reachable | <1% (chỉ dev/tech-savvy) | 100% Hùng+ TAM |

**Implication operational:**
- Single point of failure: Telegram suspend bot → toàn user offline. Mitigation: maintain backup bot token sẵn (`BOT_TOKEN_BACKUP`) cho switchover < 5 phút.
- Telegram-imposed rate limit ~30 msg/s toàn bot → ở ≥500 active users với spike (vd 23:00 daily recap) cần bot pool. Roadmap: 1 bot (0-500) → bot pool 2-5 (500-2000) → Local Bot API server self-host (2000+). Detail xem [PRD section 5.4.2](file:///Users/YourName/Projects/MyMoneyWent/docs/prd.md).
- Privacy: BOT_TOKEN compromise = read all conversations. Token rotation runbook bắt buộc, store trong secret manager (không commit, không log).

**Why not let user bring own bot?** Đã consider và reject:

| Approach | Pros | Cons |
|---|---|---|
| Shared bot (chosen) | UX 2-15 phút, scale được TAM | Single point of failure, rate limit |
...
→ **Shared bot is the only viable choice cho mass-market consumer SaaS.**
...
| 4 | **Multi-channel foundation MVP** | Telegram + Messenger (Facebook Page) — Telegram public launch, Messenger feature-flagged | **Telegram = primary launch channel.** Messenger code + foundation ship MVP (schema, adapter, webhook endpoint) nhưng public access **gated bởi `ENABLE_MESSENGER_CHANNEL` flag** — chỉ flip ON sau khi Meta App Review approve `pages_messaging` + `pages_messaging_subscriptions`. Lý do: Meta review không deterministic (3-14 ngày, có thể reject), không cho phép block MVP launch. App Review submit Tuần 10 đầu (parallel với code build) để review chạy trong khi dev. Zalo defer Phase 2 sau khi đạt 100+ user. Detail: [feature-spec-messenger-channel v1.1.1 §13 ramp stages](file:///Users/YourName/Projects/MyMoneyWent/docs/features/feature_messenger_channel.md) |
| 5 | MRR | Monthly Recurring Revenue | **$150-450** (revised từ $100-300 với pricing mới) |
...
| **Hành vi** | Dùng Telegram hàng ngày, có 1 tài khoản ngân hàng VN (TCB, Vietcombank, MB...) |
...
**Đây là primary persona cho Business tier — revenue driver thực sự của FinTrack ở mid-stage.**
...
**WTP cho FinTrack Business $9 (220k VND):**
- vs kế toán dịch vụ 500k → rẻ hơn 56% + real-time
- vs KiotViet 200k → tương đương price, simpler
- vs Excel free → tiết kiệm 4-6h/tháng = 400k value → ROI 1.8x
- 220k VND đặt **dưới ngưỡng tâm lý 250k** nhưng trên 100k để "đủ serious"
...
| 4 | **Email forwarding ingest** | User chỉ muốn email → bot cấp unique inbound address `u<id>@in.fintrack.app` → user setup forwarding rule trong Gmail/Outlook → bot parse email tự động | All |
...
| 8 | **Transaction categorization** | Inline buttons để phân loại nhanh | All |
| 9 | **Tracking mode** | Theo dõi chi tiêu theo category | All |
| 10 | **Budget mode (optional)** | Đặt ngân sách cho category, cảnh báo | All |
| 11 | **Reports** | /status, /today | All |
| 12 | **Daily recap** | Tự động gửi tổng kết cuối ngày (theo timezone) | All |
| 13 | **Multi-user isolation** | Mỗi user data riêng biệt | All |
| 14 | **Free tier limits** | 45 tx/tháng, 1 bank account, 30 ngày history, **1 email source**, 5 categories (3 auto-created defaults + 2 custom slots) | Free |
| 15 | **14-day Pro trial** | New user mặc định Pro 14 ngày, auto-downgrade | Free |
| 16 | **Pro: Multi-bank** | 3 bank accounts | Pro |
| 17 | **Pro: Reports** | Weekly + Monthly report | Pro |
| 18 | **Pro: CSV export** | Export data ra spreadsheet | Pro |
| 19 | **Pro: Email parsing 3 sources** | 3 email forwarding sources (Free = 1, Pro = 3, Business = unlimited; Free/Pro khác nhau ở tx cap 45 vs unlimited) | Pro |
...
| Feature | Mô tả |
|---|---|
| Messenger / Zalo integration | Platform thứ 2 sau Telegram |
| Auto-categorization rules ML-based | Sau khi có ≥10k tx data |
| Team/family workspace | Multi-user shared workspace |
...
| 4 | Inventory management | Wrong product, FinTrack không track tồn kho |
...
Cấu trúc **3-tier**: Free / Pro $4 / Business $9 + **14-day Pro trial cho new users**.
...
**Annual plan (20% off cho cả 2 tier):** Pro $38.40/năm (~960k VND), Business $86.40/năm (~2.16tr VND). Tương đương ~2.4 tháng free khi trả năm.
...
|---------|-------------|------|
| Minh (nhân viên văn phòng) | Free → Pro | $3-5/mo |
| Linh (freelancer) | Pro | $4-7/mo |
| Hùng+ (online seller / chủ shop) | Business | $7-12/mo |
...
**Cohort projection — steady-state ở 100 active users (cần acquire ~125-130 total để có 100 active sau churn):**

| Tier | % active users | Số users | Gross Revenue/mo | Sau 7% churn impact |
|------|---------|----------|-----------|----|
| Free | 85% | 85 | $0 | — |
| Pro ($4) | 12% | 12 | $48 | $44.6 (−$3.4 từ churn) |
| Business ($9) | 3% | 3 | $27 | $25.1 (−$1.9) |
| **Total** | 100% | 100 | $75 | **~$70/mo** |
...
| Free | 82% | 410 | $0 | — |
| Pro | 14% | 70 | $280 | $260 |
| Business | 4% | 20 | $180 | $167 |
| **Total** | 100% | 500 | $460 | **~$427/mo** |

**Implications của churn 7%:**
- Phải acquire ~25-30% extra users so với projection no-churn để giữ steady-state
- LTV Pro ở 7% churn: $4/mo ÷ 7% = ~$57 (vs $200+ nếu churn 2%)
- LTV Business: $9/mo ÷ 7% = ~$129
- → CAC payback < $30-50 cho Pro để economic make sense
...
| 3% (best case) | $446 | $133 |
| 5% (target) | $437 | $80 |
| 7% (realistic VN B2C) | $427 | $57 |
| 10% (worst case) | $414 | $40 |

→ MRR target **$427-446** ở 500 users với churn realistic. Vẫn cao hơn lower bound $300 của BRD nhưng thấp hơn projection no-churn $460.
...
- **Bank transfer + auto-detect** (primary, 0% fee) — user chuyển khoản tới platform's bank account với ref string `PAY-{user_id}-{plan}-{period}-{nonce4}` → SePay webhook detect ≤ 60s, Email parsing backup ≤ 5min. 4-layer fuzzy matching cho typo tolerance. Detail spec: [feature-spec-payment-bank-transfer.md](file:///Users/YourName/Projects/MyMoneyWent/docs/features/feature_payment.md).
...
#### 5.4.1. Break-even theo platform @ 100 users (Pro $4, MVP scope = B+C combined)

| Platform | Cost (incl. email parsing) | Paying users cần | % conversion break-even |
|---------|------|----------------|----------------------|
| **Railway Hobby + Postmark** | $25 | **7** | **7%** |
| Hetzner Singapore + Postmark | $20 | 5 | 5% |
| DO Singapore + Postmark | $42 | 11 | 11% |
| Fly.io managed + Postmark | $56 | 14 | 14% |

→ Railway @ 7% conversion realistic — vẫn dưới 10-12% conversion target. Email parsing tăng break-even threshold từ 4% → 7% nhưng đổi lại serve full TAM.

#### 5.4.2. Break-even theo scale (Railway, MVP với email parsing)

| Scale | Cost/mo | Paying users cần | % conversion |
|-------|---------|-----------------|--------------|
| 10 users | $17 | 5 | 50% (beta tight, founder + family chỉ định trả) |
| 50 users | $20 | 5 | 10% |
| 100 users | $25 | 7 | 7% |
| 200 users | $32 | 8 | 4% |
| 500 users | $73 | 19 | 4% |

**Insight quan trọng:** Beta phase (10-50 users) **break-even khó hơn nhiều** với email parsing trong MVP. Email parsing $10/mo là fixed cost — ăn nhiều vào revenue lúc users ít. Recommend: **founder pay out-of-pocket $50-100/tháng cho 3-4 tháng đầu**, accept sunk cost cho TAM coverage.
...
|-------|-----------|-------------|---------|-----|
| 100 users | $15 | 12% Pro + 3% Business | $75 | **+$60 (80%)** |
| 500 users | $40 | 14% Pro + 4% Business | $460 | **+$420 (91%)** |
...
|-------|---------|-------|---------------------------|---------------------------------------------|------------|
| 100 users | $75 | $15 | $1.4 | $200/mo (~30h × 250k) | **-$141 (LOSS, founder time = burn)** |
| 500 users | $460 | $40 | $8.3 | $400/mo (~50h × 250k) | **+$12 (3% margin nếu thuê support)** |

**C. Realistic margin nếu founder không tính lương cho mình (bootstrap mode):**

| Scale | Revenue | Infra + Fees | Net margin |
|-------|---------|--------------|------------|
| 100 users | $75 | $16.4 | **+$58.6 (78%) — nhưng founder làm support free** |
| 500 users | $460 | $48.3 | **+$411.7 (89%) — bottleneck ở support time** |

**Kết luận:**
- **Infrastructure margin >80%** chỉ valid khi founder không tính opportunity cost của mình.
- **Business margin thực tế** là thấp hơn nhiều khi tính support time. Ở 500 users với MRR $460, không thể thuê customer support full-time.
- **Profitable từ ~50 users** chỉ đúng theo infra margin, không đúng theo business margin.
...
|-----------|-------------------|----------------------|
| **Sản phẩm** | Money Lover Free, MISA Money Keeper, Sổ Thu Chi | **Money Lover + Linked Wallet, FinTrack** |
| **TAM VN** | Mass market | Niche (cần connection với bank) |
| **Pricing model** | One-time / Free | Subscription |

### 6.2. So sánh chi tiết

| Sản phẩm | Pricing | Auto-capture? | Personal/Business split? | Channel | So với FinTrack |
|----------|---------|---------------|------------------------|---------|----------------|
| **Money Lover Free** | Miễn phí | ❌ (manual) | ❌ | Mobile app | Khác category — FinTrack auto-capture |
| **Money Lover Premium** | $24.49 one-time | ❌ (vẫn manual) | ❌ | Mobile app | Premium chỉ unlock features (CSV, attach), không có auto-capture |
| **Money Lover + Linked Wallet** | Premium $24.49 + Linked Wallet sub (~$3-5/mo) | ✅ | ❌ | Mobile app | **Đây mới là direct competitor.** FinTrack chạy Telegram (no app install), có Personal/Business split |
| **MISA Money Keeper** | Free + ads | ❌ (manual) | ❌ | Mobile app | Khác category |
| **Sổ Thu Chi MISA** | Free | ❌ | ❌ | Mobile app | Khác category |
| **Excel/Sheets DIY** | Free + 4-6h/tháng | Semi (paste statement) | Manual tag | N/A | FinTrack tự động + chat UX |
| **KiotViet POS** | 200-300k/mo | ✅ (POS scope) | ❌ (POS focused) | Web + mobile | Wrong category cho online seller cá nhân |
| **Kế toán dịch vụ** | 500k-1tr/mo | Manual (kế toán nhận invoice) | ✅ (manual) | Email/Zalo | FinTrack Business: 56% rẻ hơn + real-time |

### 6.3. Direct competitor analysis: Money Lover + Linked Wallet vs FinTrack

Đây là cuộc cạnh tranh thực sự. So sánh head-to-head:

| Dimension | Money Lover + Linked Wallet | FinTrack Pro |
|-----------|------------------------------|-------------|
...
| **Channel** | Mobile app (cần install) | Telegram (đã có sẵn) |
...
→ **TCO 5 năm gần như tương đương** giữa Money Lover Linked Wallet và FinTrack Pro. Khác biệt chính ở **3 điểm:**

1. **Channel** — Telegram-native vs mobile app: lợi cho FinTrack với user đã sống trong messaging
2. **Personal/Business split** — FinTrack có (Business tier), Money Lover không
3. **Bank coverage** — Money Lover hỗ trợ direct bank, FinTrack qua SePay (cần user có SePay)
...
1. **Telegram-native trong VN context** — đa số online seller đã dùng Telegram (cộng đồng MMO, dropshipper VN active trên Telegram). FinTrack sống trong workflow đã có.
...
| Comparable | Price/year | FinTrack equivalent | Gap |
|-----------|-----------|--------------------|---|
| Money Lover Linked Wallet (auto-capture) | $36-60/năm | Pro $36-48/năm | **Gần ngang** |
| Money Lover Premium one-time + Linked Wallet | $60-84 năm 1 | Pro $36-48 năm 1 | FinTrack rẻ hơn năm đầu, ngang về sau |
| Kế toán dịch vụ | 6-12tr/năm ($240-480) | Business $84-108/năm | FinTrack **80% rẻ hơn** |
| KiotViet POS | 2.4-3.6tr/năm | Business $84-108/năm | FinTrack **70% rẻ hơn** (nhưng feature scope khác) |

→ FinTrack Pro **không phải premium pricing** so với Money Lover Linked Wallet. FinTrack Business là **major value play** so với kế toán dịch vụ và KiotViet.
...
| 2 | **Telegram block ở VN** | Thấp | Messenger code + foundation đã build từ MVP, chỉ pending Meta App Review. Nếu Telegram block xảy ra trong khi review pending — escalate Meta để approve nhanh + flip `ENABLE_MESSENGER_CHANNEL=true`. Nếu review đã pass: switchover 100% Messenger trong < 1 ngày. Worst case Meta cũng reject: pivot Zalo Phase 2 (effort ~2 tuần dev tương tự Messenger, schema + adapter pattern đã sẵn). Detail: [feature-spec-messenger-channel §11 risk #6](file:///Users/YourName/Projects/MyMoneyWent/docs/features/feature_messenger_channel.md) |
...
| 9 | **Compliance VN (Nghị định 13/2023 PDPA)** | Cao | Privacy policy rõ, data retention policy, breach response plan. PDPA breach runbook deferred — re-eval @ 200 users hoặc post-incident (xem [implementation plan §5b](file:///Users/YourName/Projects/MyMoneyWent/docs/implementation-plan-500-users-and-more.md)) |
...
| 13 | **Telegram bot suspended** (rare nhưng impactful) | Thấp-Trung bình | `BOT_TOKEN_BACKUP` env var ready. `@FinTrackUpdates` channel làm out-of-band notification path (created pre-launch). DR runbook §6 Scenario D playbook |
...
**Revised v2.7.2 (admin tools + observability scope):** Phase 6 mở rộng tuần 10-11 → **tuần 10-12** (10 ngày → 14-20 ngày work) để cover payment integration (8-12 ngày) + admin tools commands (3-5 ngày) + observability dashboard (3-5 ngày). Tổng timeline vẫn **16 tuần** vì Phase 7 dịch tuần 13-14, Phase 8 dịch tuần 15-16 (buffer week 16 absorbed). Pre-Phase 1: viết 3 spec critical — [admin tools](file:///Users/YourName/Projects/MyMoneyWent/docs/features/feature_admin_tools.md), [disaster recovery](file:///Users/YourName/Projects/MyMoneyWent/docs/runbooks/disaster-recovery.md), [observability](file:///Users/YourName/Projects/MyMoneyWent/docs/observability-plan.md) — 3-5 ngày, parallel với hộ kinh doanh registration lead time.

**Revised v2.9 (multi-channel foundation MVP + VietQR + email parallel):** Phase 6 thêm Messenger channel build (Tuần 10–11, 2 tuần dev = 10 ngày, code ship MVP nhưng public launch gated bởi feature flag) + VietQR via vietqr.io public image URL + email parallel path (**3–5 ngày** integration/test, fit Tuần 11). Tuần 12 thu hẹp scope deploy + admin tools + observability để absorb thêm 1-3 ngày VietQR/email. Foundation Phase 1 thêm schema multi-channel (`channel_type`, `channel_user_id`). Tổng timeline giữ **16 tuần** — Messenger build cover bằng cách upgrade Phase 6 từ "polish + deploy" thành "multi-channel build + polish + deploy". **Critical: Meta App Review submit Tuần 10 đầu (parallel với dev), review chạy trong 3-14 ngày — public Messenger launch sau khi approve, KHÔNG block MVP Telegram launch.** Detail: [feature-spec-messenger-channel v1.1.1 §13 ramp stages](file:///Users/YourName/Projects/MyMoneyWent/docs/features/feature_messenger_channel.md) + [implementation-plan-payment-vietqr-email](file:///Users/YourName/Projects/MyMoneyWent/docs/implementation-plan-payment-vietqr-email.md).

| Phase | Thời gian | Deliverables |
|-------|-----------|-------------|
| **Phase 1: Foundation** | Tuần 1-2 | Repo mới, DB schema (incl. `admin_audit_log`, `analytics_events`, **`users.channel_type` + `channel_user_id`** cho multi-channel), multi-user routing, **messenger.send() interface abstraction + `services/channels/` adapter pattern foundation**, Docker Compose setup |
| **Phase 2: Handlers refactor** | Tuần 3-4 | Refactor handlers → multi-user via messenger interface, auth flow, tenant isolation, **admin command authorization framework** (ADMIN_TELEGRAM_IDS), `TelegramSender` adapter implement (di chuyển code từ `telegram_api.py`) |
...
| **Phase 6: Multi-channel build + Payment + Deploy** | Tuần 10-12 | **Expanded scope**: (a) **Messenger channel build** (Tuần 10-11, 2 tuần): Facebook Page setup + **App Review submit Tuần 10 đầu (parallel review)**, `services/channels/messenger.py`, `handlers/messenger_webhook.py`, persistent menu, onboarding parity Telegram. **Code ship Tuần 11 cuối với `ENABLE_MESSENGER_CHANNEL=false` mặc định — flip ON khi App Review approve (có thể ở Phase 7 hoặc post-launch).** (b) **Payment integration** (8-12 ngày): bank transfer auto-detect SePay primary + Email backup + 4-layer matching + manual review fallback. (c) **VietQR + email parallel** (**3-5 ngày**): `services/qr_generator.py` (vietqr.io public image URL) + `send_image()` adapter cho 2 channel + email_platform_tcb parser + cross-source dedup E2E + edge cases. (d) **Admin tools commands** (3-5 ngày). (e) **Observability dashboard + alerts** (3-5 ngày). (f) Railway production deploy + domain, backup automation + DR runbook. PayPal/USDT defer Phase 2. |
| **Phase 7: Closed beta** | Tuần 13-14 | Beta 5-10 user **trên Telegram** (Messenger nếu App Review pass thì add ≥2 user; nếu chưa pass thì validate 100% Telegram), đo actual cost, iterate critical bugs, **test recovery backup full restore** (DR runbook §F.A scenario), **landing page tĩnh + privacy policy + terms ship** (~3 ngày, theo [decision-onboarding-ui-strategy §5.2 + §8](file:///Users/YourName/Projects/MyMoneyWent/docs/decision-onboarding-ui-strategy.md) — KHÔNG build web onboarding form, chỉ landing + privacy required cho Meta App Review + PDPA compliance). |
| **Phase 8: Public soft launch** | Tuần 15-16 | Mở rộng 20-30 user **trên Telegram (primary)**. Messenger flip ON nếu App Review approved + foundering accept Meta policy compliance — không block soft launch nếu chưa approve. Validate 3 onboarding path × N channel matrix (N=1 hoặc 2 tùy review status), monitor parser accuracy. Buffer absorbed nếu Phase 5/6 slip |
...
- [ ] **Admin tools commands** working end-to-end (xem [feature-spec-admin-tools v1.1.0](file:///Users/YourName/Projects/MyMoneyWent/docs/features/feature_admin_tools.md)): `/admin_help` (auto-generated), `/admin_resolve`, `/admin_refund`, `/admin_force_plan`, `/admin_stats`, `/admin_cost`. Rate limit 30/min per admin enforced.
- [ ] **`@FinTrackUpdates` Telegram channel created** pre-launch — out-of-band notification path cho bot suspension scenarios
...
- [ ] **Disaster recovery runbook** ([docs/runbooks/disaster-recovery.md](file:///Users/YourName/Projects/MyMoneyWent/docs/runbooks/disaster-recovery.md)) — 8 scenarios documented (incl. Scenario G SePay outage, Scenario H abuse/spam)
...
| Partnership SePay co-marketing | $0 (revenue share) | Tháng 4-6 | Nếu SePay không compete: proposal co-market — SePay user nhận FinTrack Pro 1 tháng free, FinTrack share 20-30% revenue back |
...
| **Email-only seller targeting** | $0 | Tháng 2-3 | Target audience không quen tech: "Bạn nhận email báo giao dịch từ ngân hàng? Forward về bot, có ngay báo cáo." Đây là segment ít competitor đụng vì cần email parsing — FinTrack có moat |
...
| Risk | Mức độ | Mitigation |
|------|------|---|
| Hùng+ không sống trên Telegram (chỉ Facebook/Zalo) | Cao | Phase 3 mở Messenger/Zalo platform — đã list trong roadmap |
...
1. Post lên 5-10 Telegram groups VN (lập trình, dev, freelancer, indie maker)
...
| Channel Identity | User identity trên Telegram (chat_id) |
...
| Tài liệu | Link |
|------|---|
| Current self-hosted repo | `/Users/YourName/Projects/Bot Finance` |
| README | `/Users/YourName/Projects/Bot Finance/README.md` |
...
| v2.8.0 | 2026-05-06 | **Sync với 3 spec mới (admin-tools v1.1.0, DR runbook v1.1.0, observability v1.1.0) + implementation plan v1.2.0:** (1) **§7 risk register expanded** từ 10 → 14 risks: thêm Risk 11 (abuse/spam attacks — DR §8c playbook), Risk 12 (founder support burnout @ 200+ — C1 automation must ship trước 250), Risk 13 (Telegram bot suspended — BOT_TOKEN_BACKUP + @FinTrackUpdates channel), Risk 14 (cost burn > revenue valley of death 100-300 users — cost dashboard + error budget). Risk 9 PDPA + Risk 10 scale cross-ref tới implementation plan §C4 + §5b. (2) **§10 success criteria** expanded thành 5 groups: Functional (+admin tools commands /admin_help auto-gen, rate limit 30/min, /admin_cost; +@FinTrackUpdates channel), Reliability (+error budget 0.1% rolling 30-day, +7 critical alerts armed, +PLATFORM_TOKEN silence monitoring), Cost (+cost dashboard, +margin >50% sustained), Onboarding (giữ), **Operational readiness mới** (DR runbook 8 scenarios, B2 password manager, SSE-B2 encryption, pg_dumpall globals, trusted contact briefed). (3) No scope/timeline change — tất cả là spec consolidation. |
| v2.9.0 | 2026-05-07 | **Multi-channel foundation MVP + VietQR + email parallel (sync feature-spec-messenger-channel v1.1.1 + impl plan VietQR+email v1.0.0):** (1) **§2.2 mục tiêu 4 changed**: Messenger từ "Phase 2 validate (chưa cần live)" → **"foundation + code ship MVP, public access feature-flagged sau Meta App Review approve"**. Single-channel per user (khi cả 2 channel live, user chọn 1 lúc onboarding). Zalo defer Phase 2. **Telegram public launch primary, không phụ thuộc Meta review.** (2) **§risk #2 update**: Telegram block mitigation đổi từ "Messenger là platform #2 validate sẵn sàng" → "Messenger code + foundation đã build từ MVP, pending Meta App Review. Nếu Telegram block xảy ra trong khi review pending — escalate Meta + flip flag. Worst case Meta cũng reject: pivot Zalo Phase 2." (3) **§8 Phase 6 timeline expand scope**: Tuần 10–11 thêm 2 tuần Messenger build (Facebook Page setup + App Review submit Tuần 10 đầu parallel với dev + adapter implement + onboarding parity), Tuần 11 thêm **3-5 ngày** VietQR via vietqr.io public image URL + email parallel path (integration/test surface lớn — không phải 2 ngày). Tuần 12 thu hẹp scope deploy + admin tools + observability để absorb. Tổng timeline vẫn 16 tuần. (4) **§8 Phase 1 deliverables**: thêm schema multi-channel (`users.channel_type`, `channel_user_id`) + `services/channels/` adapter pattern foundation. **§8 Phase 2** thêm `TelegramSender` adapter migrate từ `telegram_api.py`. (5) **§8 Phase 6 ship Tuần 11 cuối** Messenger code với `ENABLE_MESSENGER_CHANNEL=false` mặc định — flip ON khi App Review approve (có thể Phase 7 hoặc post-launch). **§8 Phase 7 beta target**: 5-10 user trên Telegram (Messenger nếu review approved thì add ≥2 user). **§8 Phase 8 soft launch**: 20-30 user trên Telegram primary; Messenger flip ON nếu approved, KHÔNG block soft launch nếu chưa. (6) Header thêm v2.9 changelog summary. |
