...
- Telegram-imposed rate limit ~30 msg/s toàn bot → ở ≥500 active users với spike (vd 23:00 daily recap) cần bot pool. Roadmap: 1 bot (0-500) → bot pool 2-5 (500-2000) → Local Bot API server self-host (2000+). Detail xem [PRD section 5.4.2](file:///Users/maingocanh/Projects/MyMoneyWent/docs/prd.md).
...
- **Bank transfer + auto-detect** (primary, 0% fee) — user chuyển khoản tới platform's bank account với ref string `PAY-{user_id}-{plan}-{period}-{nonce4}` → SePay webhook detect ≤ 60s, Email parsing backup ≤ 5min. 4-layer fuzzy matching cho typo tolerance. Detail spec: [feature-spec-payment-bank-transfer.md](file:///Users/maingocanh/Projects/MyMoneyWent/docs/features/feature_payment.md).
...
| 2 | **Telegram block ở VN** | Thấp | Messenger code + foundation đã build từ MVP, chỉ pending Meta App Review. Nếu Telegram block xảy ra trong khi review pending — escalate Meta để approve nhanh + flip `ENABLE_MESSENGER_CHANNEL=true`. Nếu review đã pass: switchover 100% Messenger trong < 1 ngày. Worst case Meta cũng reject: pivot Zalo Phase 2 (effort ~2 tuần dev tương tự Messenger, schema + adapter pattern đã sẵn). Detail: [feature-spec-messenger-channel §11 risk #6](file:///Users/maingocanh/Projects/MyMoneyWent/docs/features/feature_messenger_channel.md) |
...
| 9 | **Compliance VN (Nghị định 13/2023 PDPA)** | Cao | Privacy policy rõ, data retention policy, breach response plan. PDPA breach runbook deferred — re-eval @ 200 users hoặc post-incident (xem [implementation plan §5b](file:///Users/maingocanh/Projects/MyMoneyWent/docs/implementation-plan-500-users-and-more.md)) |
...
- [ ] **Admin tools commands** working end-to-end (xem [feature-spec-admin-tools v1.1.0](file:///Users/maingocanh/Projects/MyMoneyWent/docs/features/feature_admin_tools.md)): `/admin_help` (auto-generated), `/admin_resolve`, `/admin_refund`, `/admin_force_plan`, `/admin_stats`, `/admin_cost`. Rate limit 30/min per admin enforced.
...
- [ ] **Disaster recovery runbook** ([docs/runbooks/disaster-recovery.md](file:///Users/maingocanh/Projects/MyMoneyWent/docs/runbooks/disaster-recovery.md)) — 8 scenarios documented (incl. Scenario G SePay outage, Scenario H abuse/spam)
