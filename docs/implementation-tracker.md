...
| F-i18n | Wave 1 | F-i18n — VI/EN locale switcher | ⬜ | `feat/F-i18n` | 🔒X | **Unblocked 2026-05-13** (post-F01 + F07). Stub `i18n/vi.py` + `i18n/en.py` đã land W0.4. F07 + F01 ship với keys ở vi.py + en.py literal (parity test enforced). F-i18n PR sẽ: (1) polish EN wording cho keys hiện tại từ literal placeholders, (2) add language confirm UI (FE spec feature-onboarding.md v1.2.0 §3 Language Selection), (3) expand stub → full `t(locale, key)` module với fallback rules, (4) update F01/F07 handlers chuyển sang `t(user.locale, ...)`. Lockdown decision 2026-05-13 trong docs/operations/F01-F08-lockdown.md §3.1: F-i18n owns full EN parity. Trước đây mark ❌ là legacy status — không có blocker mechanical thực sự. |
