...
| work-state-engine-1b | Wave 0 follow-up | Work-State Engine — Phase 1b (github + ci + railway collectors) | ⬜ | `feat/MYM-3-work-state-engine-1b` | 🔒I 🔒X | **Ready to kickoff post MYM-1 merge.** **Linear:** [MYM-3](https://linear.app/maingocanh/issue/MYM-3) — workspace `maingocanh`, project `Work-State Engine`, milestone `Phase 1b — GitHub + CI + Railway collectors`, P1 manual_only. **Spec:** `docs/operations/dashboard-engine/dashboard-plan-state-split.md` v1.2.1 §6.3 + §6.4 + §6.5 + §6.7 + §7.4 + §8.1 + §13. **Prereq:** MYM-1 Phase 1a merged (5072e9e 2026-05-20). **Prompt:** `docs/autopilot/prompts/work-state-engine-phase-1b-autopilot.md` (refactored 2026-05-20 per A+ scope reconcile: 14 Steps + 3 Checkpoints + 13 circuit breakers incl. RAILWAY_REGRESSION). **Scope:** 3 new collectors (`github.py` + `ci.py` + `railway.py`) + Signals dataclass APPEND-ONLY +9 fields (pr/ci/review/deploy state) + status_machine APPEND priorities for deploy_state §8.1. PR identity §6.3 5-step fallback. Cache TTL (5min PR list / 1min detail / 30s CI / 1min railway) + `--no-network` mode + unknown-safe network. 2-3 work-days. Codex 2× consecutive clean required. Out of scope: projection (MYM-4 Phase 1b'), multi-branch agg (Phase 1c), urgency (Phase 1d), CI enforcement (Phase 2). |
| work-state-engine-1b' | Wave 0 follow-up | Work-State Engine — Phase 1b' (dashboard projection follow-up) | ⏸️ | `feat/MYM-4-work-state-engine-1b-projection` | 🔒X | **Gated by MYM-3 (Phase 1b) merge.** **Linear:** [MYM-4](https://linear.app/maingocanh/issue/MYM-4) — project `Work-State Engine`, P1 manual_only. **Spec:** `docs/operations/dashboard-engine/dashboard-plan-state-split.md` v1.2.1 §13 AC1g + AC11a-c. **Prereq:** MYM-3 merged (Signals dataclass extensions stable, github+ci+railway active). **Prompt:** TBD — draft sau khi MYM-3 ships + Signals schema stable. **Scope:** Single module `scripts/work_state/projections/dashboard.py` — enrich `docs/dashboard.json` rows với `state` block (computed_status + pr_state + ci_state + deploy_state + review_state + overlays + last_event_ts). Side-by-side rendering (manual `status` từ tracker giữ UNCHANGED, additive `state` từ engine). CLI entry point. Idempotent. `--no-network` mode supported. ~0.5-1 work-day. Codex 2× consecutive clean required. Out of scope: multi-branch agg projection (Phase 1c), manual status removal (Phase 3 cutover). **Scope reconcile rationale:** A+ option lock 2026-05-20 — projection split out of spec §10's canonical 1b để align Linear milestone 1:1 với prompt scope. |
