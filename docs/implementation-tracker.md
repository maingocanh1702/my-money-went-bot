...
| security-batch-B | Wave 0 follow-up | Security hardening — 9 fixes (H5, H7, M1-M7, SSRF) | ✅ | `feat/c1-p5-retire-legacy` | 🔒X | **Merged `30e6a4e` via squash PR #21 (2026-05-22).** 9 fixes: capture path hardening (M1+M2), observability cleanup (M3+M5+M6+M7), hygiene+config+supply chain, SSRF. |
| sepay-multi-tenant | Wave 0 follow-up | SePay multi-tenant C1 P1-P3 (DB pool+Sentry wire, webhook route, parallel-run telemetry) | ✅ | `feat/c1-p1/p2/p3` | 🔒T 🔒X | **Merged PRs #23/#24/#25 (2026-05-15).** `POST /webhooks/sepay/{token}` multi-tenant route + DB pool + Sentry + structlog + request_id at startup + parallel-run telemetry on legacy + v2. |
| zalo-channel-core | Wave 0 follow-up | Zalo channel migration 0004 + start handler routing | ✅ | main (direct) | 🔒T 🔒M | **Committed `26cf1a5` (2026-05-28).** Migration 0004: `channel_chat_id TEXT NULL` + `chk_channel_type` +zalo. Start handler accepts `channel_chat_id`. User dataclass + `_row_to_user()` updated. |
| zalo-sender-webhook | Zalo V1 | Zalo sender + webhook route + categorize handler | 🟡 | (untracked) | 🔒I 🔒T 🔒X | **Local WIP.** `core/messenger/zalo.py` ZaloSender, `core/handlers/zalo_webhook.py` route, `core/handlers/categorize.py` DB queue, `core/services/bot_state.py`. Pending commit + live Zalo OA fixture verification. Implementation plan v0.6.0. |
...
| W1.3 | n/a | Phase 1 integration smoke | ⬜ | `chore/W1.3-phase1-smoke` | 🔒T 🔒X | E2E test: 3 channels (TG+Zalo+Discord) → 3 users → tenant isolated |

**Phase 1 exit criteria:** Docker compose lên 1 lệnh, Discord + Zalo + Telegram cùng pass contract test, smoke E2E xanh.
...
|-------|:---------:|:------:|:-----------:|:-------:|:--------:|:-:|
| 1 | 19 | 16 (W0.7-W0.10, WSE 1a-1d, DashLive A/B, MYM-8/11, Security B, SePay MT, Zalo core) | 1 (zalo-sender-webhook) | 0 | 0 | 84% |
...
| 6 | 10 | 0 | 0 | 0 | 0 | 0% |
| **MVP total** | **48** | **18** | **1** | **0** | **0** | **38%** |

(Wave 0 = 6 PRs đã merged, không count vào MVP remaining. Phase 1 expanded significantly với Work-State Engine (8 PRs), Dashboard Live View (2 PRs), security batch B, SePay multi-tenant C1, and Zalo channel rows added v1.5.0. Phase 5b = 3 parsers deferred, unlock per demand signal. Phase 2 total 10 với manual-transaction-entry row added 2026-05-15.)
...
| v1.5.0 | 2026-05-30 | **Major sync (17 days of shipped work).** Added rows: security-batch-B (✅ `30e6a4e` 2026-05-22, 9 fixes), sepay-multi-tenant (✅ PRs #23-#25 2026-05-15, C1 P1-P3), zalo-channel-core (✅ `26cf1a5` 2026-05-28, migration 0004 + start routing), zalo-sender-webhook (🟡 local WIP). Phase 1: 7→19 total PRs, 4→16 merged (84%). Phase 2 unchanged (10 PRs, 2 merged, 20%). MVP total: 36→48 PRs, 6→18 merged (17%→38%). W1.3 smoke test scope expanded TG+Discord → TG+Zalo+Discord. Phase 1 exit criteria updated to include Zalo contract test. Work-State Engine rows (MYM-1/3/4/5/6/7/8/10) already existed from prior tracker auto-updates — this version reconciles the summary counts that lagged behind. |
