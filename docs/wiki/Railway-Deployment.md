...
Open Railway project settings and add these variables. **The app refuses to start in production unless every required variable below is set** — the webhooks handle money, so an unauthenticated endpoint is treated as a configuration error rather than a default.
...
| `EMAIL_SECRET` | Same value as the `SECRET` in `google_apps_script.js`. |
| `CRON_SECRET` | Random string for `/trigger/*` endpoints. |

Zalo channel — optional, adds a second channel (see [Zalo Setup](Zalo-Setup)):

| Variable | Notes |
|---|---|
| `ZALO_ENABLED` | `true` to enable Zalo. |
| `ZALO_BOT_TOKEN` | Zalo Bot Platform token. |
| `ZALO_CHAT_ID` | Your Zalo chat id (discover with `scripts/zalo_get_updates.py`); also the only accepted sender. |
| `ZALO_SECRET_TOKEN` | Matches the `X-Bot-Api-Secret-Token` header Zalo sends. Required when `ZALO_ENABLED=true`. |
