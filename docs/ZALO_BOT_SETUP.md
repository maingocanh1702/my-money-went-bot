...
```env
ZALO_SECRET_TOKEN=<random secret 8-256 chars>
ZALO_CHAT_ID=<user_id từ scripts/zalo_get_updates.py>
```
...
| Bot không start | Nếu `ZALO_ENABLED=true` thì bắt buộc có `ZALO_BOT_TOKEN`, `ZALO_CHAT_ID` và `ZALO_SECRET_TOKEN` |
| `/zalo/webhook` không xử lý message | Kiểm tra webhook secret đúng và `ZALO_CHAT_ID` đúng |
