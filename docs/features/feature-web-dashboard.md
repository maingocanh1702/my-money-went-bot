...
> **Tham chiếu:** [BRD-vi v3.3.0 §4.3](file:///Users/maingocanh/Projects/MyMoneyWent/docs/brd-vi.md) · [PRD-vi v1.8.0 §1.2](file:///Users/maingocanh/Projects/MyMoneyWent/docs/prd-vi.md) · [Assessment](file:///Users/maingocanh/Projects/MyMoneyWent/docs/research/webapp-resource-assessment.md) · [Roadmap v1.3.0 Phase W](file:///Users/maingocanh/Projects/MyMoneyWent/docs/mymoneywent-roadmap.md)
>
...
|---|-------|-----------|---------|
| 1 | User (Free) | Nhấn link "Xem Dashboard" trong bot → mở web | Thấy transaction list 30 ngày gần nhất |
| 2 | User (Free) | Filter theo category | Danh sách lọc theo category đã chọn |
| 3 | User (Free) | Xem category monthly summary | Hiện pie chart + spending breakdown |
| 4 | User (Pro) | Filter theo date range + funding source | Full history filtered |
| 5 | User (Pro) | Xem monthly trend chart | Bar chart spending theo tháng |
...
| 9 | User | Magic link expired → click lại | Thấy "Link hết hạn, request link mới từ bot" |
| 10 | User | Truy cập dashboard không có link | Redirect tới landing page + hướng dẫn mở từ bot |

### Edge Cases

| # | Category | Case | Xử lý |
|---|----------|------|-------|
| 1 | Security | Magic link sử dụng lần 2 | Reject — single-use only |
| 2 | Security | Magic link hết TTL (>10 phút) | Reject + thông báo request mới |
| 3 | Security | User A dùng magic link User B | Reject — link bound to user_id |
| 4 | Security | Brute force magic link token | Rate limit + 404 response (no info leak) |
| 5 | Data | User không có transaction nào | Empty state: "Chưa có giao dịch nào. Kết nối ngân hàng qua bot để bắt đầu!" |
| 6 | Data | Free user xem >30 ngày | Blur/lock + upgrade CTA |
| 7 | Data | User có 10,000+ transactions | Pagination (50 per page), lazy load |
| 8 | Concurrency | User đang xem, bot nhận tx mới | Dashboard refresh khi user navigate (không realtime push MVP) |
| 9 | Data Integrity | Transaction deleted via bot | Dashboard reflects current DB state on next load |
| 10 | Cross-Feature | User downgrade Pro→Free đang xem full history | Session stays until refresh, then enforce 30d limit |
...
> **Note:** Screen designs deferred tới Phase W implementation planning. Below is structural spec only.
...
| Error | "Link hết hạn hoặc không hợp lệ. Mở bot để nhận link mới." |
| Empty | N/A |
...
| Error | "Không thể tải dữ liệu. Thử lại sau." + retry CTA |
| Empty | "Chưa có giao dịch nào. Kết nối ngân hàng qua bot!" + icon + CTA |
...
| Error | Error banner + retry |
| Empty | "Cần ít nhất 2 tháng dữ liệu để hiện trend." |
...
## 4. Domain Model (shared — xem PRD §4)

Dashboard reads từ existing tables, KHÔNG thêm table mới ngoại trừ:

| Table | Mô tả | Mới? |
|-------|-------|:----:|
| dashboard_sessions | JWT refresh token store + audit log (refresh hash, expires_at, revoked_at) | 🆕 |
| dashboard_magic_links | Single-use auth tokens, TTL, used_at | 🆕 |
...
| dashboard_page_view | User navigates to page | user_id, page (transactions/categories/charts/settings) |
| dashboard_filter_used | User applies filter | user_id, filter_type (date/category/source) |
| dashboard_export_csv | User exports CSV (Pro+) | user_id, row_count |
| dashboard_tier_gate_hit | Free user hits Pro feature | user_id, feature (full_history/charts/export) |
...
- [ ] Magic links have **short TTL** (≤10 minutes)
...
- [ ] All auth events audited (dashboard_link_generated, _used, _expired)
...
|---------|------|----------|
| v1.0.0 | 2026-05-13 | Initial feature spec. Scope locked: read-only companion dashboard. 10 use cases, 12 edge cases, 5 screens × 4 states, 2 new tables, 9 analytics events, P1 auth security requirements. Implementation deferred (trigger-based). Synced with BRD v3.3.0, PRD v1.8.0, Roadmap v1.3.0. |
