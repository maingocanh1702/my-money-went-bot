...
def get_styles():
    ...
    styles.add(ParagraphStyle(
        'ClusterTitle', parent=styles['Heading2'],
        fontSize=14, leading=18, textColor=ACCENT,
        spaceBefore=16, spaceAfter=8, fontName='Helvetica-Bold'
    ))
    styles.add(ParagraphStyle(
        'AppName', parent=styles['Heading3'],
    ...
    styles.add(ParagraphStyle(
        ...
        leftIndent=16, spaceAfter=2, fontName='Helvetica-Oblique'
    ))
    styles.add(ParagraphStyle(
        'Takeaway', parent=styles['Normal'],
        fontSize=9, leading=12, textColor=HIGHLIGHT,
        spaceBefore=4, spaceAfter=6, fontName='Helvetica-Bold'
    ))
    styles.add(ParagraphStyle(
        'SmallGray', parent=styles['Normal'],
        fontSize=8, leading=10, textColor=GRAY, spaceAfter=2
    ))
    styles.add(ParagraphStyle(
    ...
    styles.add(ParagraphStyle(
        ...
        fontSize=7.5, leading=9.5, textColor=white, fontName='Helvetica-Bold'
    ))
    return styles


def hr():
    return HRFlowable(width="100%", thickness=0.5, color=HexColor("#dee2e6"), spaceBefore=6, spaceAfter=6)
...
def build_app_card(story, s, name, tagline, price, users, rating, markets, hq,
                   pricing_rows, how_rows, strengths, weaknesses,
                   quotes, target, similarity, match_attrs, takeaway):
...
def build():
    ...
    cover_info = [
        ...
        ['Match criteria', '(a) Chat UX, (b) Non-Plaid auto, (c) <=5$/mo, (d) P+B split, (e) Multi-currency 5+'],
        ['Clusters', '1: Generic apps | 2: Messaging bots | 3: Email/SMS parsing | 4: Solopreneur lite'],
    ...
    master_headers = ['#', 'App', 'Platform', 'Entry $/mo', 'Free?', 'Auto-capture', 'P+B Split', 'Multi-curr', 'Users(M)', 'Rating']
    master_rows = [
        ...
        ['8', 'TeleExpense', 'Telegram', '$1 1x', 'N', 'Manual>Sheets', 'N', '[?]', '<0.005', 'N/A'],
        ...
        ['12', 'Walnut/axio', 'Android', '$0', 'Y', 'SMS(deprecated)', 'N', 'N(INR)', '5.0+', '[?]'],
        ...
        ['19', 'Notion Tpl', 'Web/All', '$0-$39 1x', 'Y', 'Manual', 'Y(tpl)', 'Y(man)', 'N/A', 'N/A'],
    ]
    story.append(make_table(master_headers, master_rows,
                            col_widths=[18, 58, 48, 48, 28, 68, 38, 42, 40, 30]))
    story.append(PageBreak())
    ...
    build_app_card(story, s,
        name="MONEY LOVER (Finsify)",
        tagline="SEA's dominant manual expense tracker",
        price="From $2.49/mo", users="[?]M users", rating="4.6 (iOS)",
        markets="SEA/Global", hq="Hanoi, Vietnam",
        pricing_rows=[
            ['Free (Basic)', '$0', '--', '--', '2 wallets, 1 budget, ads'],
            ['Premium', '[?]', '[?]', '[?]', 'Unlimited wallets/budgets, no ads'],
            ['Linked Wallet', '$2.49', '~$14/yr', '~$1.17', 'Salt Edge bank sync (separate add-on)'],
        ...
        similarity="3/5", match_attrs="non-Plaid auto / cheap-pricing / multi-currency",
    ...
    build_app_card(story, s,
        ...
        price="From $1.99/mo", users="3.0M users", rating="4.6 (iOS)",
        markets="Global/EU-focus", hq="Prague, Czech Republic",
        pricing_rows=[
    ...
    build_app_card(story, s,
        ...
        price="$5.99 one-time", users="22M downloads", rating="4.64 (Android)",
        markets="Global/Android", hq="South Korea",
        pricing_rows=[
        ...
        similarity="2/5", match_attrs="cheap-pricing / multi-currency",
    ...
    build_app_card(story, s,
        ...
        price="$7.99/mo", users="6M downloads", rating="4.06 (Android)",
        markets="Global/Android only", hq="Unknown (indie dev)",
        pricing_rows=[
        ...
        similarity="1/5", match_attrs="multi-currency only",
    ...
    build_app_card(story, s,
        ...
        price="$4.99 one-time", users="[?]", rating="3.33 (And Lite)",
        markets="US/Global", hq="Anishu Inc.",
        pricing_rows=[
        ...
        strengths=['One-time purchase, cross-platform', 'Family sync (device-to-device OTA)', 'Multi-currency live FX'],
        weaknesses=['Per-OS purchase expensive', 'No bank sync, 100% manual', 'Low rating 3.33 = UX issues'],
        quotes=[],
    ...
    build_app_card(story, s,
        ...
        price="$0 (freemium)", users="1M downloads", rating="[?]",
        markets="Global/Android", hq="FerApps",
        pricing_rows=[
        ...
        strengths=['90+ currencies', 'Unlimited custom categories', 'Recurring transactions & templates'],
        weaknesses=['Last update July 2023 -- 22 months stale', 'Unknown aggregator', 'Android-only'],
    ...
    # --- CLUSTER 2 ---
    story.append(Paragraph("CLUSTER 2: Messaging-first / Chatbot finance (PRIORITY CAO)", s['ClusterTitle']))
    story.append(hr())

    build_app_card(story, s,
        name="PIGGYPAL (Telegram bot)",
        tagline="Most polished Telegram finance bot",
        price="$0 (free)", users="10-100K est.", rating="N/A (bot)",
        markets="Global", hq="Unknown (indie)",
        pricing_rows=[
        ...
        similarity="4/5", match_attrs="chat-UX / non-Plaid auto(OCR) / cheap-pricing / multi-currency",
    ...
    build_app_card(story, s,
        ...
        price="$1 one-time", users="<5K est.", rating="N/A (bot)",
        markets="Global", hq="Unknown (indie)",
        pricing_rows=[
        ...
        strengths=['Data ownership (user controls Sheets)', 'One-time pricing = trust signal', 'Simple, focused'],
        weaknesses=['$1 price = economically non-viable', 'Sheets = no rich viz', 'No bank integration'],
        quotes=[],
        target="Privacy-conscious / DIY finance",
        similarity="3/5", match_attrs="chat-UX / cheap-pricing / (MMW also has Sheets sync)",
    ...
    build_app_card(story, s,
        ...
        price="Freemium", users="<10K est.", rating="N/A (bot)",
        markets="Global", hq="Unknown (solo founder)",
        pricing_rows=[
        ...
        strengths=['Group budgeting = unique moat', 'Multi-currency FX auto-conversion', '~5 years old, stable'],
        weaknesses=['No bank integration', 'Solo founder = limited scale', 'No retention data disclosed'],
        quotes=[],
        target="Groups: flatmates / couples / travel friends",
        similarity="3/5", match_attrs="chat-UX / cheap-pricing / multi-currency",
    ...
    build_app_card(story, s,
        ...
        price="Freemium", users="2-5M active", rating="~4.0 (iOS)",
        markets="US/UK", hq="London, UK",
        pricing_rows=[
        ...
        similarity="2/5", match_attrs="chat-UX (originally) / non-Plaid auto (now Plaid)",
    ...
    build_app_card(story, s,
        ...
        price="$0", users="5M+ installs", rating="[?]",
        markets="India", hq="Bangalore, India",
        pricing_rows=[
        ...
        strengths=['Proved SMS auto-capture works technically', 'Acquired for ~$30M (2018)', '5M+ installs'],
        weaknesses=[
        ...
        similarity="3/5", match_attrs="non-Plaid auto (SMS) / cheap-pricing",
    ...
    build_app_card(story, s,
        ...
        price="$0", users="75M downloads", rating="1.8-2.0",
        markets="India", hq="Bangalore, India",
        pricing_rows=[
        ...
        strengths=['75M downloads -- massive scale', 'Profitable NBFC', 'SMS auto-capture at scale'],
        weaknesses=[
        ...
        similarity="2/5", match_attrs="non-Plaid auto (SMS) / cheap-pricing",
    ...
    build_app_card(story, s,
        ...
        price="$9.99/mo", users="2.5M users", rating="4.8 (iOS)",
        markets="UK/Global", hq="United Kingdom",
        pricing_rows=[
        ...
        strengths=['22K+ 5-star ratings', 'UKs fastest-growing Gen Z budget app', 'Shared budget for couples'],
        weaknesses=['Manual entry only', '$9.99/mo > MMW $9', 'No personal+business split'],
        quotes=['Fastest-growing budgeting app on UK App Store for Gen Z -- startupsmagazine.co.uk'],
        target="UK Gen Z / couples",
        similarity="2/5", match_attrs="cheap-pricing(annual) / multi-currency",
    ...
    build_app_card(story, s,
        ...
        price="$9.99/mo", users="[?]", rating="4.5",
        markets="US/Global", hq="United States",
        pricing_rows=[
            ['Free', '$0', '--', '--', 'Basic tracking'],
            ['Premium', '$9.99', '$99.99', '$8.33', 'Auto-track income/expense/mileage, 1000+ integrations, tax est.'],
        ],
        how_rows=[
        ...
        similarity="4/5", match_attrs="non-Plaid auto / cheap-pricing / personal-biz-split",
    ...
    build_app_card(story, s,
        ...
        price="$24/mo ($17 annual)", users="[?]", rating="4.5 (Capterra)",
        markets="US/Global", hq="San Francisco, US",
        pricing_rows=[
        ...
        strengths=['All-in-one: contracts, proposals, invoicing, CRM', 'Professional templates', 'Strong freelancer brand'],
        weaknesses=['$24/mo min = 2.7x MMW price', 'No personal finance', 'Feature bloat for solos'],
    ...
    build_app_card(story, s,
        ...
        price="$0 forever", users="2M+ users", rating="4.4",
        markets="US/CA/Global", hq="Toronto, Canada",
        pricing_rows=[
        ...
        strengths=['Free forever -- no catch for core', '2M+ users, established brand', 'Invoicing + receipt + multi-user'],
        weaknesses=['Business-only -- no personal finance', 'Revenue from payments, not SaaS', 'Manual entry'],
        ...
        similarity="1/5", match_attrs="cheap-pricing (free)",
    ...
    build_app_card(story, s,
        ...
        price="$0-$39 one-time", users="N/A", rating="N/A",
        markets="Global", hq="Various creators",
        pricing_rows=[
        ...
        weaknesses=['ZERO automation -- kills adoption month 2', 'No bank/OCR/parsing', 'High churn after initial enthusiasm'],
        ...
        similarity="2/5", match_attrs="cheap-pricing / personal-biz-split(manual)",
    ...
    story.append(Paragraph("Rank theo do giong MMW (5 attributes: Chat UX / Non-Plaid Auto / <=5$/mo / P+B Split / Multi-currency)", s['SmallGray']))
    story.append(Spacer(1, 8))

    sim_headers = ['Rank', 'App', 'Chat UX', 'Non-Plaid', '<=5$/mo', 'P+B Split', 'Multi-curr', 'Total']
    sim_rows = [
    ...
    story.append(make_table(sim_headers, sim_rows,
                            col_widths=[30, 70, 45, 60, 50, 50, 50, 40]))
    story.append(Spacer(1, 12))

    story.append(Paragraph('<b>Top 3 Direct Threats to Monitor:</b>', s['SubHead']))
    story.append(Paragraph('1. <b>PiggyPal (4/5)</b> -- Same platform (Telegram), same UX (chat), free. If adds bank sync + monetize = real threat. Currently hobby project.', s['BulletCustom']))
    story.append(Paragraph('2. <b>Hurdlr (2-3/5)</b> -- Same personal+business split at same price. Different platform (native app). Most feature-competitive. Mobile-only weakness.', s['BulletCustom']))
    story.append(Paragraph('3. <b>Cointry + TeleExpense (3/5)</b> -- Same platform, cheap. Low overlap but if either raises VC = arms race.', s['BulletCustom']))
    ...
    story.append(Paragraph('12-18 months truoc khi VC discovers Telegram finance TAM va funds Cointry/PiggyPal. First professional player to scale wins. Act with urgency -- Sept 2026 launch critical.', s['Body']))
    ...
    adv_rows = [
        ...
        ['3-tier value ladder ($0/$4/$9)', 'Strong', 'Competitors: free or $1 one-time (no growth)'],
        ['Non-Plaid auto-capture', 'Differentiator', 'Avoids reliability issues of Plaid/Salt Edge'],
    ]
    story.append(make_table(adv_headers, adv_rows, col_widths=[150, 80, 270]))

    story.append(Spacer(1, 16))
    story.append(hr())
    story.append(Paragraph('Report generated: May 2026 | 4 parallel research agents | 19 apps analyzed | 30+ verified sources', s['SmallGray']))
