"""Sacombank email parser shell. F02 fills the parse() body."""
...
_SENDER_RE = re.compile(r"@sacombank\.com(\.vn)?$", re.IGNORECASE)
_AMOUNT_RE = re.compile(r"([\d,.]+)\s*(?:VND|đ)", re.IGNORECASE)


@register_parser("SACOMBANK")
class SacombankParser(BankEmailParser):
    ...
    def parse(self, email: InboundEmail) -> CanonicalTx:
        ...
        if not match:
            raise ValueError("SacombankParser: no amount found")
        amount = int(match.group(1).replace(",", "").replace(".", ""))
        return CanonicalTx(
            source="email_sacombank",
            bank="SACOMBANK",
