"""BIDV email parser shell. F02 fills the parse() body."""
...
_SENDER_RE = re.compile(r"@bidv\.com\.vn$", re.IGNORECASE)
_AMOUNT_RE = re.compile(r"([\d,.]+)\s*(?:VND|đ)", re.IGNORECASE)


@register_parser("BIDV")
class BIDVParser(BankEmailParser):
    ...
    def parse(self, email: InboundEmail) -> CanonicalTx:
        ...
        if not match:
            raise ValueError("BIDVParser: no amount found")
