...
def test_resolver_email_hangseng(fake_ss):
    _seed_account(fake_ss, "hsbc_main",
                  ["email_hangseng:REDACTED-a-real-Hang-Seng-account"], currency="HKD")
    res = resolve_account({"_source": "email_hangseng", "_account_hint": "REDACTED-a-real-Hang-Seng-account"})
    assert res.status == "matched"
