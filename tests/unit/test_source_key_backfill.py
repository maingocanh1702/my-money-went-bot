...
stores the raw source_key (e.g. "sepay:REDACTED-a-real-bank-account-number") in col U even
...
def test_append_transaction_writes_source_key_to_col_u(fake_ss):
    ...
    sh.append_transaction(
        ...
        account_source_key="sepay:REDACTED-a-real-bank-account-number",
    )
    ws = sh._sheet(S.TRANSACTIONS)
    row = ws.row_values(2)
    assert row[16] == ""                      # account_id still empty
    assert row[20] == "sepay:REDACTED-a-real-bank-account-number"     # but col U has source_key
...
def test_backfill_assigns_account_id_to_matching_rows(fake_ss):
    _seed_tx_table(fake_ss)
    # 3 rows for sepay:REDACTED-a-real-bank-account-number (no account_id yet) +
    ...
    n = sh.backfill_account_id_by_source_key(
        account_id="REDACTED-a-real-account-slug",
        source_key="sepay:REDACTED-a-real-bank-account-number",
    )
    assert n == 2  # only the 2 unmapped rows with matching source

    rows = sh._sheet(S.TRANSACTIONS).get_all_values()
    assert rows[1][16] == "REDACTED-a-real-account-slug"           # was empty → filled
    assert rows[2][16] == "REDACTED-a-real-account-slug"           # was empty → filled
...
async def test_commit_with_source_backfills_historical_tx(
    ...
    state = {
        ...
        "pending_source_key": "sepay:REDACTED-a-real-bank-account-number",
        "pending_setup_key":  "",
        "pending_identifier": "REDACTED-a-real-bank-account-number",
        "new_acct_row_num":   0,
        "pending_account": {
            "name": "TPB 2601", "id": "REDACTED-a-real-account-slug",
    ...
    acc = sh.find_account_by_id("REDACTED-a-real-account-slug")
    assert acc is not None
    assert "sepay:REDACTED-a-real-bank-account-number" in acc["source_keys"]

    # Historical tx now bear the account_id
    rows = sh._sheet(S.TRANSACTIONS).get_all_values()
    assert rows[1][16] == "REDACTED-a-real-account-slug"
    assert rows[2][16] == "REDACTED-a-real-account-slug"
