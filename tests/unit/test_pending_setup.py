...
def test_add_pending_writes_row(fake_ss):
    ...
    setup_key = sh.add_pending_account(
        ...
        identifier="REDACTED-a-real-bank-account-number",
        tx_row_num=42,
    )
    assert setup_key
    ...
    assert rows[1][1] == "sepay:REDACTED-a-real-bank-account-number"
    assert rows[1][2] == "REDACTED-a-real-bank-account-number"
    assert rows[1][3] == "42"
...
def test_stale_pending_row_is_flipped_and_new_row_appended(fake_ss):
    ...
    setup_key = sh.add_pending_account("sepay:REDACTED-a-real-bank-account-number", "REDACTED-a-real-bank-account-number", tx_row_num=1)
    ...
    setup_key_2 = sh.add_pending_account("sepay:REDACTED-a-real-bank-account-number", "REDACTED-a-real-bank-account-number", tx_row_num=99)
...
async def test_prompt_new_account_writes_sheet_not_state(fake_ss, monkeypatch):
    ...
    await prompt_new_account("sepay:REDACTED-a-real-bank-account-number", "REDACTED-a-real-bank-account-number", tx_row_num=2)
