...
async def test_assign_shows_preview_with_buttons(
    ...
    _seed_account(fake_ss, "REDACTED-a-real-account-slug", currency="VND")
    _seed_tx(fake_ss, [
    ...
    await accounts._cmd_accounts_assign("REDACTED-a-real-account-slug")

    assert "REDACTED-a-real-account-slug" in sent["text"]
    ...
    assert "asg_yes_REDACTED-a-real-account-slug" in cbs
    ...
    assert state.get("assign_slug") == "REDACTED-a-real-account-slug"
...
async def test_assign_no_candidates_when_all_mapped(
    ...
    _seed_account(fake_ss, "REDACTED-a-real-account-slug")
    _seed_tx(fake_ss, [
        ["t1", "2026-05-01T10:00:00", "", "", "", "x", "Tiền ra", "100000",
         "r1", "0", "", "", "FALSE", "FALSE", "2026-05", "VND", "REDACTED-a-real-account-slug"],
    ])
    sent = []
...
async def test_assign_confirm_writes_account_id_to_rows(
    ...
    sh.set_state(CHAT_ID, {
        ...
        "assign_slug": "REDACTED-a-real-account-slug",
        "assign_rows": [2, 3],
    })
    ...
    await accounts.handle_assign_callback(["asg", "yes", "REDACTED-a-real-account-slug"], message_id=99)

    ws = sh._sheet(S.TRANSACTIONS)
    rows = ws.get_all_values()
    # Col Q (index 16) on rows 2 and 3 should now be "REDACTED-a-real-account-slug"
