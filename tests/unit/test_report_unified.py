...
TX_HEADER = [
    ...
    "Currency", "account_id", "ledger_tx_type",
]
...
def _seed_tx(fake_ss, rows: list[list]):
    ...
    for i, r in enumerate(rows):
        ...
        ws.update(f"A{i+2}:R{i+2}", [padded])
...
@freeze_time(FROZEN_NOW_UTC)
def test_report_excludes_transfer_and_cc_payment_from_spending(fake_ss):
    _seed_buckets(fake_ss, [{"id": "food", "name": "Food", "allocated": 0}])
    _seed_accounts(fake_ss, [
        {"id": "tpb", "name": "TPB", "type": "bank"},
        {"id": "cake", "name": "Cake Visa", "type": "credit"},
    ])
    today = _iso_local(2026, 5, 20)
    _seed_tx(fake_ss, [
        ["t1", today, "", "", "", "food", "Tiền ra", "100000", "r1", "0",
         "food", "", "FALSE", "TRUE", "2026-05", "VND", "tpb", "expense"],
        ["t2", today, "", "", "", "move", "Tiền ra", "1000000", "r2", "0",
         "", "", "FALSE", "TRUE", "2026-05", "VND", "tpb", "transfer"],
        ["t3", today, "", "", "", "pay cc", "Tiền ra", "500000", "r3", "0",
         "", "", "FALSE", "TRUE", "2026-05", "VND", "tpb", "cc_payment"],
        ["t4", today, "", "", "", "external cc pay", "Tiền vào", "200000", "r4", "0",
         "", "", "FALSE", "TRUE", "2026-05", "VND", "cake", "cc_payment"],
    ])

    d = _scan_period("m")

    assert d["total_out"] == 100000
    assert d["total_in"] == 0
    assert d["by_account"][0]["out"] == 100000
    assert d["by_bucket"][0]["spent"] == 100000
    flows = {(f["tx_type"], f["currency"]): f for f in d["internal_flows"]}
    assert flows[("transfer", "VND")]["amount"] == 1_000_000
    assert flows[("cc_payment", "VND")]["amount"] == 700_000
    rendered = _render_account_lens(d)
    assert "Chuyển khoản: 1.000.000đ (1 tx)" in rendered
    assert "Trả CC: 700.000đ (2 tx)" in rendered
