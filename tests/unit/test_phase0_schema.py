...
def test_accounts_tab_created_with_correct_header(fake_ss):
    ...
    # 15 cols per plan §2.1 + 1 (col P starting_outstanding, added so the
    # credit's opening declared debt survives every update_account_cache
    # recompute — without it the cache would zero out the starting balance
    # on the first ledger write).
    assert len(sh.ACCOUNTS_HEADER) == 16
    assert sh.ACCOUNTS_HEADER[-1] == "starting_outstanding"
