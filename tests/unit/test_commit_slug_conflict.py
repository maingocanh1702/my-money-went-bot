...
Production bug (2026-05-25): user created `REDACTED-a-real-account-slug` via `/accounts add`
(no source). Later a SePay tx for sepay:REDACTED-a-real-bank-account-number arrived → resolver
returned new_identifier → prompt fired → user ran wizard naming the
account "TPB 2601" again → slug `REDACTED-a-real-account-slug` collision → _commit cancelled.
...
async def test_commit_slug_exists_with_source_links_to_existing(
    ...
    sh.add_account(
        account_id="REDACTED-a-real-account-slug", name="TPB 2601", acc_type="bank",
    ...
    # triggered new_identifier for sepay:REDACTED-a-real-bank-account-number, user re-typed the
    # same account name.
    state = {
        ...
        "pending_account": {
            "name": "TPB 2601",
            "id":   "REDACTED-a-real-account-slug",  # slug collides with existing
            "type": "bank",
            "currency": "VND",
            "starting_balance": 0,
        },
    ...
    res = resolve_account({"accountNumber": "REDACTED-a-real-bank-account-number"})
    assert res.status == "matched"
    assert res.account_id == "REDACTED-a-real-account-slug"
...
async def test_commit_slug_exists_without_source_still_errors(
    ...
    state = {
        ...
        "pending_account": {
            "name": "TPB 2601",
            "id":   "REDACTED-a-real-account-slug",
            "type": "bank",
            "currency": "VND",
            "starting_balance": 0,
        },
