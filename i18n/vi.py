...
STRINGS: dict[str, str] = {
    ...
    "cb.mcc_updated":       "✅ Đã chuyển pattern `{pattern}` sang MCC {mcc}.",
    "cb.mcc_deleted":       "🗑️ Đã tắt pattern `{pattern}`. Dòng vẫn còn trong sheet (active=FALSE).",
    "cb.mcc_not_found":     "⚠️ Không thấy pattern `{pattern}` đang bật. Xem lại danh sách ở trên.",
    "cb.mcc_del_need_input": "⚠️ Cần `del <pattern>` (vd `del WINMART`).",
    "cb.mcc_renamed":       "✏️ Đổi keyword `{old}` → `{new}`. MCC và nhãn giữ nguyên.",
    "cb.mcc_ren_need_input": "⚠️ Cần `ren <cũ> <mới>` (vd `ren WIMART WINMART`).",
    "cb.mcc_ren_conflict":  "⚠️ Đã có pattern `{pattern}` đang bật. Xoá một trong hai trước (`del <pattern>`), rồi đổi tên.",
    "cb.mcc_ren_same":      "ℹ️ `{pattern}` vốn đã là tên đó rồi.",
    "cb.mcc_skipped":       "🚫 `{pattern}` sẽ không tính cashback nữa, và bot cũng thôi hỏi về nó.",
    "cb.mcc_skip_exists":   "ℹ️ `{pattern}` vốn đã nằm trong danh sách không hoàn tiền.",
    "cb.mcc_skip_need_input": "⚠️ Cần `skip <pattern>` (vd `skip TIKTOKSHOP`).",
    "cb.mcc_unskipped":     "✅ Bỏ chặn `{pattern}` — giao dịch của nó tính cashback lại bình thường.",
    "cb.mcc_skip_not_found": "⚠️ `{pattern}` không nằm trong danh sách không hoàn tiền.",
    "cb.mcc_unskip_need_input": "⚠️ Cần `unskip <pattern>` (vd `unskip TIKTOKSHOP`).",
    "cb.mcc_bad_code":      "⚠️ `{mcc}` không phải mã MCC. MCC là 4 chữ số — vd `WINMART 5411 Siêu thị`. Nếu tên pattern có khoảng trắng thì bỏ khoảng trắng đi (vd `TIKTOKSHOP`).",
