...
STRINGS: dict[str, str] = {
    ...
    "cb.mcc_updated":       "✅ Moved pattern `{pattern}` to MCC {mcc}.",
    "cb.mcc_deleted":       "🗑️ Turned off pattern `{pattern}`. The row stays in the sheet (active=FALSE).",
    "cb.mcc_not_found":     "⚠️ No active pattern `{pattern}`. Check the list above.",
    "cb.mcc_del_need_input": "⚠️ Need `del <pattern>` (e.g. `del WINMART`).",
    "cb.mcc_renamed":       "✏️ Renamed `{old}` → `{new}`. Its MCC and label are unchanged.",
    "cb.mcc_ren_need_input": "⚠️ Need `ren <old> <new>` (e.g. `ren WIMART WINMART`).",
    "cb.mcc_ren_conflict":  "⚠️ `{pattern}` is already an active pattern. Remove one of them first (`del <pattern>`), then rename.",
    "cb.mcc_ren_same":      "ℹ️ `{pattern}` is already spelled that way.",
    "cb.mcc_skipped":       "🚫 `{pattern}` will not earn cashback, and the bot will stop asking about it.",
    "cb.mcc_skip_exists":   "ℹ️ `{pattern}` is already on the no-cashback list.",
    "cb.mcc_skip_need_input": "⚠️ Need `skip <pattern>` (e.g. `skip TIKTOKSHOP`).",
    "cb.mcc_unskipped":     "✅ `{pattern}` is off the no-cashback list — its transactions are scored again.",
    "cb.mcc_skip_not_found": "⚠️ `{pattern}` is not on the no-cashback list.",
    "cb.mcc_unskip_need_input": "⚠️ Need `unskip <pattern>` (e.g. `unskip TIKTOKSHOP`).",
    "cb.mcc_bad_code":      "⚠️ `{mcc}` is not an MCC code. An MCC is four digits — e.g. `WINMART 5411 Groceries`. If your pattern has a space in it, remove the space (e.g. `TIKTOKSHOP`).",
