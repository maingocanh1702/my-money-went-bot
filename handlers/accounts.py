...
from config import ZALO_CHAT_ID, ZALO_ENABLED
import messenger
import sheets as sh
...
async def prompt_new_account(source_key: str, identifier: str, tx_row_num: int):
    ...
    # (e.g. source_key="sepay:REDACTED-a-real-bank-account-number" or "email_cake:cake_cc").
    msg = t("ac.unmapped", masked=masked, source=source_key)
    buttons = [[
        {"text": t("ac.btn_setup"), "callback_data": f"acc_setup_{setup_key}"},
        {"text": t("ac.btn_skip"),  "callback_data": f"acc_skip_{setup_key}"},
    ]]
    await tg.send_with_buttons(msg, buttons)


async def prompt_zalo_new_account(source_key: str, identifier: str, tx_row_num: int):
    """Send a Zalo text-menu onboarding prompt for an unmapped account/card.

    Zalo Bot Platform has no Telegram-style callback buttons, so the setup key is
    carried in a plain text command: `setup <key>` or `skip <key>`.
    """
    ...
    if not setup_key:
        print("[zalo accounts] prompt skipped (empty source_key)")
        return

    masked = _mask(identifier)
    await messenger.send_text(
        f"Tài khoản/thẻ chưa liên kết: {masked}\n"
        f"nguồn: {source_key}\n\n"
        "Bot chưa nhận diện tài khoản/thẻ này.\n"
        f"Reply: setup {setup_key}\n"
        f"Hoặc: skip {setup_key}\n\n"
        "Cài đặt còn hiệu lực 24h.",
        channel="zalo",
...
async def _start_setup(message_id: int, setup_key: str):
    ...
    await tg.edit_message(
        message_id,
        t("ac.setup_title", masked=masked, step=1, total=2),
    )
...
async def handle_new_account_name(text: str, state: dict):
    ...
    buttons = [[
        ...
        {"text": "🧾 Credit", "callback_data": "acc_type_credit"},
    ...
    await tg.send_with_buttons(t("ac.step_type", step=2, total=2, slug=pending['id']), buttons)


async def _on_type_picked(acc_type: str, message_id: int):
    """VND-only — currency step skipped entirely. bank/debit/cash commit
    immediately (name → type → commit). credit opens the extra setup steps
    (limit → outstanding → statement day → due day) shared with the Zalo
    wizard, so a credit card is created identically on either channel.
    """
    if acc_type not in ("bank", "debit", "cash", "credit"):
        return
    state = _state()
    pending = state.get("pending_account") or {}
    ...
    if acc_type == "credit":
        ...
        await tg.send_text(t("ac.step_limit", step=3, total=6))
        return
    # bank/debit/cash: 3-step wizard ends here.
...
CREDIT_DUE_PROMPT = None  # computed dynamically via t()


def _credit_statement_prompt():
    return t("ac.step_statement", step=5, total=6)


def _credit_due_prompt():
    return t("ac.step_due", step=6, total=6)


def parse_billing_day(text: str) -> tuple[int | None, bool, str | None]:
    """Parse a statement/due day from free text. Channel-agnostic.

    Returns (day, ok, error):
      - 'skip' / '-' / '' / '0'  → (None, True, None)  intentionally unset
      - a valid 1–28 day         → (day, True, None)
      - anything else            → (None, False, message)

    1–28 only: days 29–31 don't exist every month, so they'd make the cycle
    boundary ambiguous (same bound `set_billing_cycle` enforces).
    """
    s = (text or "").strip().lower()
    if s in ("", "skip", "-", "0", "bỏ qua", "bo qua"):
        return None, True, None
    if not s.isdigit():
        return None, False, "⚠️ Nhập một số ngày 1–28, hoặc 'skip'."
    d = int(s)
    if not 1 <= d <= 28:
        return None, False, "⚠️ Ngày phải trong 1–28 (tránh 29–31 vì không phải tháng nào cũng có)."
    return d, True, None
...
async def handle_credit_limit(text: str, state: dict):
    ...
    await tg.send_text(t("ac.step_outstanding", step=4, total=6))


async def handle_credit_outstanding(text: str, state: dict):
    """Outstanding starts here; /cc pay later decrements it. Then ask the
    ...
    _set({"pending_account": pending, "step": "await_credit_statement"})
    await tg.send_text(_credit_statement_prompt())
...
async def handle_credit_statement(text: str, state: dict):
    ...
    _set({"pending_account": pending, "step": "await_credit_due"})
    await tg.send_text(_credit_due_prompt())


async def handle_credit_due(text: str, state: dict):
    """Due day (display-only reminder). Last credit step → commit."""
    day, ok, err = parse_billing_day(text)
    if not ok:
        await tg.send_text(err)
        return
...
async def _commit(state: dict):
    ...
    async with sh.account_lock:
        ...
        if existing_by_source:
            ...
            bind_message = (
                f"ℹ️ Tài khoản `{account_id}` đã có sẵn (onboarding song song) — dùng cái cũ."
            )
        else:
            ok = sh.add_account(
            ...
            if ok:
                ...
                if slug_match and source_key:
                    ...
                    bind_message = (
                        f"🔗 Tài khoản `{account_id}` đã có sẵn — đã link thêm nguồn "
                        f"`{source_key}` vào tài khoản này."
                    )
                else:
    ...
    if bind_message:
        ...
        summary = (
            f"✅ Tài khoản *{pending['name']}* đã cài đặt\n"
            f"  · slug: `{account_id}`\n"
            f"  · loại: {pending['type']} · {pending['currency']}\n"
        )
        if source_key:
...
async def _cmd_accounts_assign(slug: str):
    ...
    if not slug:
        await tg.send_text(
            ...
            "Vd: `/accounts assign REDACTED-a-real-account-slug`\n\n"
    ...
    if not acc:
        await tg.send_text(
            f"⚠️ Tài khoản `{slug}` không tồn tại. /accounts để xem danh sách."
        )
        return
...
async def cmd_transfer(text: str):
    ...
    if not from_acc:
        await tg.send_text(f"⚠️ Tài khoản `{from_id}` không tồn tại. /accounts để xem danh sách.")
        return
    if not to_acc:
        await tg.send_text(f"⚠️ Tài khoản `{to_id}` không tồn tại.")
        return
    if from_acc["currency"] != to_acc["currency"]:
        await tg.send_text(
            f"⚠️ Loại tiền không khớp: {from_acc['currency']} → {to_acc['currency']}. "
