...
class TestSingleBranchUrgency:
    def test_normal_no_overlays(self, tracker_path: Path, tmp_dashboard: Path) -> None:
        ...
        with (
            patch(
                "work_state.engine.collect_filesystem_signals",
                return_value=_mock_signals_normal(),
            ),
            patch(
                "work_state.engine.collect_git_signals",
                return_value={
            ...
            patch("work_state.engine.collect_github_signals", return_value=_mock_gh_signals()),
            patch("work_state.engine.collect_ci_signals", return_value=_mock_ci_signals()),
            patch("work_state.engine.collect_railway_signals", return_value=_mock_railway()),
        ):
    ...
    def test_critical_deploy_failed(self, tracker_path: Path, tmp_dashboard: Path) -> None:
        ...
        with (
            ...
            patch(
                "work_state.engine.collect_github_signals",
                return_value=_mock_gh_signals(pr_state="merged"),
            ),
            patch("work_state.engine.collect_ci_signals", return_value=_mock_ci_signals()),
            patch(
                "work_state.engine.collect_railway_signals",
...
class TestMultiBranchUrgencyAggregation:
    def test_max_normal_and_elevated(self, tracker_path: Path, tmp_dashboard: Path) -> None:
        ...
        with (
            ...
            patch("work_state.engine.collect_github_signals", return_value=_mock_gh_signals()),
            patch("work_state.engine.collect_ci_signals", side_effect=mock_ci_side_effect),
            patch("work_state.engine.collect_railway_signals", return_value=_mock_railway()),
        ):
            states = run_engine(str(tracker_path), tmp_dashboard, no_network=True)
        assert len(states) == 1
