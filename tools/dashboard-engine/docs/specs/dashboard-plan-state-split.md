...
- **Spec/tech hash tracking (Phase B)**: `spec_hash` = SHA256 hex of spec file content (None if missing). `spec_modified_at` = file mtime as ISO date string. Same for `tech_hash` + `tech_modified_at`. Hash change between engine runs → emit `spec_modified` / `tech_modified` event.
- **Tracker row semantic hash (Phase B)**: `tracker_row_hash` = SHA256 of canonical serialization of semantic fields only: `(branch, linear_id, feature_id, specs, acceptance)`. Excludes `status`, `notes`, changelog, formatting whitespace. Prevents false-positive drift on auto-flip status changes.
...
| Spec file content hash changed | filesystem | `spec_modified` | overlay `spec-modified` (annotation); `post-ship-doc-change` if terminal |
| Tech spec file content hash changed | filesystem | `tech_modified` | overlay `tech-modified` (annotation); `post-ship-doc-change` if terminal |
| Tracker row semantic hash changed | filesystem | `tracker_row_modified` | overlay `tracker-modified` (annotation); `post-ship-doc-change` if terminal |
...
| `spec-modified` | spec file content hash changed since last engine collect | Spec edited — verify alignment with implementation | (annotation) |
...
**Emission status (v1.5.0 reconcile):** All 21 overlays are declared in `CANONICAL_OVERLAYS` frozenset. Emit logic for `multi-branch`, `deploy-in-progress`, `no-ci` is **reserved for future wiring** (declared in code but not yet emitted by collectors/aggregators). Listing them in §8.2 legitimizes the declarations and locks the contract — future tickets wire emission against this spec. Per memory `feedback_check_architecture_before_pattern_borrow`, do NOT remove from code until emission wired and verified.
...
- Add overlays per canonical enum §8.2: `blocked`, `stale`, `ci-running`, `ci-failing`, `review-requested`, `deploy-failed`, `unknown`, `artifact-drift`, `ambiguous-pr-mapping`, `partial-progress`, `stale-cache`, `cache-warmup`, `risk-tier-inferred`, `manual-override`, `spec-modified`, `tech-modified`, `tracker-modified`, `post-ship-doc-change`.
...
- [ ] **AC10** — Overlays implemented separately from base status. §8.2 is canonical source of overlay enum (**21 overlays**: `blocked`, `stale`, `ci-running`, `ci-failing`, `review-requested`, `deploy-failed`, `unknown`, `artifact-drift`, `ambiguous-pr-mapping`, `partial-progress`, `stale-cache`, `cache-warmup`, `risk-tier-inferred`, `manual-override`, `spec-modified`, `tech-modified`, `tracker-modified`, `post-ship-doc-change`, `multi-branch`, `deploy-in-progress`, `no-ci`). All overlay references trong spec phải dùng tên từ §8.2 (no inventing new overlay names). Subset references OK theo context:
  ...
  - §8.0 human projection maps overlays → human status (annotation-only overlays like `cache-warmup`, `multi-branch`, `no-ci` don't change human status, only render as badge)
  - §10 Phase 2 implementation list = full enum (all 21 must be implementable)
...
|---|---|---|---|
| v1.5.0 | 2026-05-21 | Founder + Claude | **MYM-11 spec reconcile (Option A per Linear ticket recommendation).** §8.2 overlay enum extended 18 → 21 — added `multi-branch` (multi-branch aggregation marker per §4.1), `deploy-in-progress` (Railway/CI deploy in flight, distinct from base `deploying` for multi-branch contexts), `no-ci` (no CI workflow detected for branch). AC10 list updated 18 → 21. Note added: emit logic for these 3 reserved for future wiring (declared in `CANONICAL_OVERLAYS` but not yet emitted by collectors); §8.2 now legitimizes the contract. Restores code ↔ spec parity (`CANONICAL_OVERLAYS` frozenset 21 = §8.2 table 21). Additive changes only — backwards compatible. Spec amendment chose over Option B (code removal) because `multi-branch` overlay was flagged in MYM-11 Linear ticket as "needed by projection to mark aggregated rows" — Option B blast radius medium, Option A blast radius zero. |
